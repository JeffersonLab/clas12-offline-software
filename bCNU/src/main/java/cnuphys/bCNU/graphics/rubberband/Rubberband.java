...
public final class Rubberband {
	 ...
	 * 
	 * @param container
	 *            the parent component being rubberbanded
	 * @param rubberbanded
	 *            who gets notified when we are done.
	 * @param policy
	 *            the stretching shape policy.
	 */
	public Rubberband(IContainer container, IRubberbanded rubberbanded,
			Policy policy) {
	 ...
	 * 
	 * @param component
	 *            the component being rubberbanded.
	 */
	...
	/**
	 * Draw the rubber band outline. Erasing is not necessary because we are
	...
	private void draw(Graphics2D g) {
		...
		case RECTANGLE_PRESERVE_ASPECT:
			if (_xorMode) {
				if (clearWindow != null) {
					clearWindow.setRubberBandRectangle(
							_container.getComponent(), rect);
				}
			} else {
				g.fillRect(rect.x, rect.y, rect.width, rect.height);
				GraphicsUtilities.drawHighlightedRectangle(g, rect,
						_highlightColor1, _highlightColor2);
			}
			break;

		case LINE:
			GraphicsUtilities.drawHighlightedLine(g, _startPt.x, _startPt.y,
					_currentPt.x, _currentPt.y, _highlightColor1,
					_highlightColor2);
			break;

		case OVAL:
			g.fillOval(rect.x, rect.y, rect.width, rect.height);
			GraphicsUtilities.drawHighlightedOval(g, rect, _highlightColor1,
					_highlightColor2);
			break;
		...
		case RADARC:
			...
			if (_tempPoly.npoints == 1) {
				GraphicsUtilities.drawHighlightedLine(g, _tempPoly.xpoints[0],
						_tempPoly.ypoints[0], _currentPt.x, _currentPt.y,
						_highlightColor1, _highlightColor2);
				...
				g.fillArc(_tempPoly.xpoints[0] - pixrad, _tempPoly.ypoints[0]
						- pixrad, size, size, startAngle, arcAngle);
				GraphicsUtilities.drawHighlightedArc(g, _tempPoly.xpoints[0]
						- pixrad, _tempPoly.ypoints[0] - pixrad, size, size,
						startAngle, arcAngle, _highlightColor1,
						_highlightColor2);

				GraphicsUtilities.drawHighlightedLine(g, _tempPoly.xpoints[0],
						_tempPoly.ypoints[0], _tempPoly.xpoints[1],
						_tempPoly.ypoints[1], _highlightColor1,
						_highlightColor2);
				GraphicsUtilities.drawHighlightedLine(g, _tempPoly.xpoints[0],
						_tempPoly.ypoints[0], (int) x2, (int) y2,
						_highlightColor1, _highlightColor2);
			}
		...
		case POLYLINE:
			tpoly = new Polygon(_tempPoly.xpoints, _tempPoly.ypoints,
					_tempPoly.npoints);
			addPoint(tpoly, _currentPt.x, _currentPt.y);
			GraphicsUtilities.drawHighlightedPolyline(g, tpoly.xpoints,
					tpoly.ypoints, tpoly.npoints, _highlightColor1,
					_highlightColor2);
			break;
		...
		case POLYGON:
			...
			addPoint(tpoly, _currentPt.x, _currentPt.y);
			g.fillPolygon(tpoly);
			GraphicsUtilities.drawHighlightedShape(g, tpoly, _highlightColor1,
					_highlightColor2);
			break;
		}
	}
	...
	public void startRubberbanding(Point anchorPt) {
		...
		if (!_xorMode) {
			...
			clearWindow = new RubberBandClearWindow(_container.getComponent(),
					0.5f);
	...
	public Rectangle getRubberbandBounds() {
		...
		if (polyMode() || radArcMode()) {
			...
			return new Rectangle((_currentPt.x < _startPt.x) ? _currentPt.x
					: _startPt.x, (_currentPt.y < _startPt.y) ? _currentPt.y
					: _startPt.y, Math.abs(_currentPt.x - _startPt.x),
