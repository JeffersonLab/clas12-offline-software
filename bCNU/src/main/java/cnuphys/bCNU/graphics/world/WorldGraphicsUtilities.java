...
public class WorldGraphicsUtilities {
	...
	if (lineColor != null) {
	    Stroke oldStroke = g2.getStroke();
	    Stroke newStroke = GraphicsUtilities.getStroke(lineWidth,
		    lineStyle);
	    g2.setStroke(newStroke);
	    g2.setColor(lineColor);
	    g2.draw(path);
	...
	drawWorldRadArc(g, container, xc, yc, rmin, rmax, startAngle, stopAngle,
	...
	GraphicsUtilities.drawHighlightedRectangle(g, rectangle, color1,
		color2);
    }
     ...
     * @param dv additional vertical offset.
     */
    public static void drawWorldText(Graphics g, IContainer container, double x,
	    double y, String text, int dh, int dv) {
	if (text == null) {
	    return;
	}
     ...
     */
    public static Polygon drawPath2D(Graphics g, IContainer container,
	    Path2D.Double path, Color fillColor, Color lineColor, int lineWidth,
	    LineStyle lineStyle, boolean closed) {
	...
	while (!pi.isDone()) {
	    ...
	    case PathIterator.SEG_LINETO:
		if (count == index) {
		    Point2D.Double wp = new Point2D.Double(coords[0],
			    coords[1]);
		    return wp;
		}
		count++;
	...
	return new Point2D.Double(p1.x + t1 * (p2.x - p1.x),
		p1.y + t1 * (p2.y - p1.y));
    }
     ...
     * @param azimuth the rotation of the ellipse in degrees. 0 is north, 90
     *            east, etc.
