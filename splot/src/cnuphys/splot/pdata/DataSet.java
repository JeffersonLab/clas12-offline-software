 ...
 * 
 */
public class DataSet extends DefaultTableModel implements
	XmlPrintStreamWritable {
     ...
     *            the dataset type
     * @param colNames
     *            the column names. There should be one name for every expected
	...
	case XYEXYE:
		...
		_columns.add(new DataColumn(DataColumnType.X, colNames[j]));
		_columns.add(new DataColumn(DataColumnType.Y, colNames[j + 1]));
		_columns.add(new DataColumn(DataColumnType.YERR,
			colNames[j + 2]));
		getColumn(j + 1).initStyle();
		getColumn(j + 1).initFit();
	    }
	...
	case XYEEXYEE:
		...
		_columns.add(new DataColumn(DataColumnType.X, colNames[j]));
		_columns.add(new DataColumn(DataColumnType.Y, colNames[j + 1]));
		_columns.add(new DataColumn(DataColumnType.XERR,
			colNames[j + 2]));
		_columns.add(new DataColumn(DataColumnType.YERR,
			colNames[j + 3]));
		getColumn(j + 1).initStyle();
     ...
     * 
     * @param xname
     *            the name for the new xdata (ignored for some types)
     * @param yname
	...
	switch (getType()) {
	case XYY:
	    System.err.println("[sPlot] Can not add curve for type: "
		    + getType());
	    break;
	...
	switch (getType()) {
	case XYY:
	    System.err.println("[sPlot] Can not add to curve for type: "
		    + getType());
	    break;
	...
	case XYXY:
		...
		System.err
			.println("[sPlot] Curve index out of range for type: "
				+ getType());
	    }
	    break;
	...
	if (is1DHistoSet()) {
		...
		for (DataColumn yc : ycols) {
			...
			System.err
				.println("All Y columns in a HistoDataset must be hitograms");
			System.exit(1);
		    }
		}
	    }
	    ...
	    System.err
		    .println("Should not be asking for histos in a non-histo dataset.");
	    System.exit(1);
	}
	return v;
    }
	...
	if (is1DHistoSet()) {
	    System.err
		    .println("Should not be calling \"getDataMin\" for histo datasets");
	    System.exit(1);
	}
	...
	if (is1DHistoSet()) {
	    System.err
		    .println("Should not be calling \"getDataMax\" for histo datasets");
	    System.exit(1);
	}
     ...
     * 
     * @param vals
     *            a variable number of entries for the columns
     */
	...
	String s = asInt ? DoubleFormat.doubleFormat(val, 0) : DoubleFormat
		.doubleFormat(val, 4, 2);
	return s;
    }
