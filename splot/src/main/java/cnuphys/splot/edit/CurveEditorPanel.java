...
public class CurveEditorPanel extends JPanel implements ActionListener,
    ...
    // list font
    protected static Font _listFont = Environment.getInstance().getCommonFont(
	    12);
    protected static Font _textFont = Environment.getInstance().getCommonFont(
	    10);
	...
	_fitPanel.getNumBinSelector().addPropertyChangeListener(this);
	...
	if (Environment.getInstance().isLinux()) {
	    _textArea
		    .setText("<body style=\"font-size:10px;color:blue\">CNU sPlot</body>");
	} else {
	    _textArea
		    .setText("<body style=\"font-size:11px;color:blue\">CNU sPlot</body>");
	}
	...
	if (source == _stylePanel.getSymbolSelector()) {
	    ...
	    SymbolType stype = SymbolType.getValue((String) ecb
		    .getSelectedItem());
	    ...
	    EnumComboBox ecb = (EnumComboBox) source;
	    LineStyle lineStyle = LineStyle.getValue((String) ecb
		    .getSelectedItem());
	...
	else if (FitEditorPanel.POLYNOMIALORDERPROP.equals(evt
		.getPropertyName())) {
	...
	else if (FitEditorPanel.BINNUMPROP.equals(evt.getPropertyName())) {
	    int nbin = (Integer) evt.getNewValue();
	    HistoData hd = curve.getHistoData();
	    if (hd.getNumberBins() != nbin) {
		curve.getFit().setDirty();
		hd.rebin(nbin);
