...
public class Grid extends JFrame implements IValueGetter {
	...
	for (int index = 0; index < 6; index++) {
	    try {
		PlotCanvas canvas = new PlotCanvas(createDataSet(index),
			getPlotTitle(index), getXAxisLabel(index),
			getYAxisLabel(index));
	...
	for (int row = 0; row < 2; row++) {
		...
		System.out.println("Plot at row: " + row + " col: " + col
			+ "  has title: "
	...
	case 3:
	    String names3[] = { "X1", "Y1", "E1", "X2", "Y2", "E2", "X3", "Y3",
		    "E3" };
	    return names3;
	...
	case 0:
		...
		try {
		    ds.add(ErfTest._rawdata[i], ErfTest._rawdata[i + 1],
			    ErfTest._rawdata[i + 2]);
		} catch (DataSetException e) {
		    e.printStackTrace();
		    System.exit(1);
		}
	...
	case 1:
		...
		try {
		    ds.add(LineWithXAndYErrors.x[i], LineWithXAndYErrors.y[i],
			    LineWithXAndYErrors.xSig[i],
	...
	switch (index) {
	case 0:
	    Collection<DataColumn> ycols = ds
	    ...
	    canvas.getDataSet().getCurveStyle(0)
