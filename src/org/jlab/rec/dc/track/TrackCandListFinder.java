...
public class TrackCandListFinder {
	...
	public void removeOverlappingTracks(List<Track> trkcands) {
		...
		list.add(trkcands.get(0));
		for(int i =1; i<trkcands.size(); i++) {
			if(trkcands.get(i).get(0).get_Segment1().get(0).get_AssociatedHBTrackID()==trkcands.get(i-1).get(0).get_Segment1().get(0).get_AssociatedHBTrackID()) {
