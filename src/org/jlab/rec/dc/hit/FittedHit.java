...
public class FittedHit extends Hit implements Comparable<Hit> {
	...
	public void set_TimeToDistance(double cosTrkAngle) {
		...
		if(_TrkgStatus!=-1) { 
			...
			//System.out.println(d+" alpha "+Math.toDegrees(Math.acos(cosTrkAngle))+" --> "+tde.interpolateOnGrid(0, Math.acos(cosTrkAngle), this.get_Time())/this.get_Time());
			//d = tde.interpolateOnGrid(0, Math.acos(cosTrkAngle), this.get_Time())/this.get_Time();
