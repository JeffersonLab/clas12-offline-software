...
public class MagneticFields {
	 ...
	 * @param result the result vector
	 */
	public void field(FieldType ftype, float x, float y, float z, CartesianVector result) {
	...
	public void initializeMagneticFields() {
		...
		if (_torusFullPath != null) {
			...
			if (file.exists()) {
				try {
					...
					System.err.println("TORUS map not found in [" + _torusFullPath + "]");
				}
			}
		}
		...
		if (_solenoidFullPath != null) {
			System.out.println("Will try to read solenoid from [" + _solenoidFullPath + "]");
			...
			if (file.exists()) {
				try {
					...
					System.err.println("SOLENIOD map not found in [" + _solenoidFullPath + "]");
				}
			}
		}
	 ...
	 */
	public File findDirectory(String root, String baseName, int maxLevel) {
	...
	private File findDirectory(String root, String baseName, int currentLevel, int maxLevel) {
	...
	public JMenu getMagneticFieldMenu(boolean incRotatedField, boolean includeTestFields) {
		...
		// menu.addSeparator();
		// _uniformItem = createRadioMenuItem(_uniform, "Constant 5 Tesla
		// Field", menu, bg, al);
		// _perfectSolenoidItem = createRadioMenuItem(_perfectSolenoid,"Perfect
		// 5 Tesla Solenoid", menu, bg, al);
		// _macTestItem = createRadioMenuItem(_macTest, "Mac's 1/r Test Field",
		// menu, bg, al);
		// }
	...
	private void compositeTest() {
		...
		double yMax = 50;
		double zMax = 300;
		CartesianVector b1 = new CartesianVector();
		CartesianVector b2 = new CartesianVector();
		...
		for (int j = 0; j < N2; j++) {
			...
			for (int i = 0; i < N1; i++) {
				...
				if ((absdif(b1.x, b2.x) > TOLERANCE) || (absdif(b1.y, b2.y) > TOLERANCE)
						|| (absdif(b1.z, b2.z) > TOLERANCE)) {
