...
public class Test_Phylo_wpgma {
  ...
  public void test() {
    ...
    // Test Phylo_wpgma
    System.out.println("Testing Phylo_wpgma");
...
//    System.out.println("Root: %f\n", tree.root);
    ...
    if (localflag) {
      fail("*** Phylo_wpgma: Tree does not have expected number of nodes");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_wpgma: Root node does not contain all the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_wpgma: Left side of tree does not report half the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_wpgma: Right side of tree does not report half the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_wpgma: Mother distance of root is not zero");
...
//      System.out.printf(tree.t[tree.t[mother].ldau].modist << " %f\n", tree.t[tree.t[mother].rdau].modist);
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_wpgma: Left and right daughter of a leaf reported different mother distances");
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_wpgma: For this tree, all mother distances should be 0.5");
...
//      System.out.printf(double(ndif)/2.0);
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_wpgma: Hamming distance between two daughters is not twice the mother distance");
...
//    System.out.println("Check expected order of leafs on tree:");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_wpgma: Leaves of the tree were not encountered in the expected order");
