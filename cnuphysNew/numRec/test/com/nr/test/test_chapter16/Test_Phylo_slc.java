...
import com.nr.ci.Phylo_slc;

public class Test_Phylo_slc {
  ...
  public void test() {
    ...
    // Test Phylo_slc
    System.out.println("Testing Phylo_slc");
...
//      System.out.printf(endl;
    ...
    Phylo_slc tree=new Phylo_slc(dist);
...
//    System.out.println("Root: %f\n", tree.root);
    ...
    if (localflag) {
      fail("*** Phylo_slc: Tree does not have expected number of nodes");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_slc: Root node does not contain all the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_slc: Left side of tree does not report half the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_slc: Right side of tree does not report half the elements");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_slc: Mother distance of root is not zero");
...
//      System.out.printf(tree.t[tree.t[mother].ldau].modist << " %f\n", tree.t[tree.t[mother].rdau].modist);
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_slc: Left and right daughter of a node reported different mother distances");
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_slc: For this tree, all mother distances should be 0.5");
...
//      System.out.printf(double(ndif)/2.0);
      ...
      globalflag = globalflag || localflag;
      if (localflag) {
        fail("*** Phylo_slc: Hamming distance between two daughters is not twice the mother distance");
...
//    System.out.println("Check expected order of leafs on tree:");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** Phylo_slc: Leaves of the tree were not encountered in the expected order");
