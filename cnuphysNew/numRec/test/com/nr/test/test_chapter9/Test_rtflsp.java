...
import static com.nr.root.Roots.rtflsp;
...
public class Test_rtflsp {
  ...
  public void test() {
    ...
    for (i=0;i<nroot;i++) {
      ...
      root[i]=rtflsp(fx,xb1[i],xb2[i],xacc);
    }
    System.out.printf("rtflsp: Maximum discrepancy = %f\n", maxel(vecsub(root,yy)));
    localflag = maxel(vecsub(root,yy)) > xacc;
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** rtflsp: Incorrect roots");
  ...
  class Func_rtflsp implements UniVarRealValueFun{
    Bessjy b = new Bessjy();
