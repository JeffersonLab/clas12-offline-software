...
public class MagneticFields {
     ...
     * 
     * @param fullPath
     *            the full path to the torus field map.
     */
     ...
     * 
     * @param fullPath
     *            the full path to the solenoid field map.
     */
    ...
    /**
     * Tries to load the magnetic fields from fieldmaps in clasJlib
     */
	...
	if (_torusFullPath != null) {
	    System.out.println("Will try to read torus from [" + _torusFullPath
		    + "]");
	...
	if (_solenoidFullPath != null) {
		...
		System.out.println("Read solenoid from: " + _solenoidFullPath);
	    } catch (FileNotFoundException e) {
		e.printStackTrace();
	    }
	}
	...
	if ((_torus == null) || (_solenoid == null)) {
	    ...
	    String clasJlib = clasJlibDir.getPath();

	    // OK, see if we can create a Torus (if we don't have one yet)
	    if (_torus == null) {
		String torusFileName = clasJlib
			+ "/data/torus/v1.0/clas12_torus_fieldmap_binary.dat";
		File torusFile = new File(torusFileName);
		try {
		    _torus = Torus.fromBinaryFile(torusFile);
		} catch (FileNotFoundException e) {
		    e.printStackTrace();
		}
	    }

	    // OK, see if we can create a Solenoid (if we don;t have one yet)
	    if (_solenoid == null) {
		String solenoidFileName = clasJlib
			+ "/data/solenoid/v1.0/solenoid-srr.dat";
		File solenoidFile = new File(solenoidFileName);
		try {
		    _solenoid = Solenoid.fromBinaryFile(solenoidFile);
		} catch (FileNotFoundException e) {
		    e.printStackTrace();
		}
	    }
	} // torus and or solenoid were null
	...
	// System.out
	// .println(String
	// .format("Test field (should be 1.018, 0, 2.234): %6.3f, %6.3f, %6.3f T",
    ...
    // locate the clasJlib folder. First look where it is expected in a relative
    // sense, then look 8 levels deep from the home directory.
    public static File getClasJlibDir() {

	File file = new File("../clasJlib");

	if (file.exists() && file.isDirectory()) {
	    System.out.println("Found clasJlib at: " + file.getPath());
	    return file;
	} else {
	    System.out.println("Did NOT find clasJlib at: " + file.getPath());
	}

	file = new File("../../clasJlib");
	if (file.exists() && file.isDirectory()) {
	    System.out.println("Found clasJlib at: " + file.getPath());
	    return file;
	} else {
	...
	System.out.println("Will look for it.");

	String homeDirectory = System.getProperty("user.home");

	file = findDirectory(homeDirectory, "clasJlib", 8);
	if (file != null) {
	    System.out.println("Found clasJlib at: " + file.getPath());
	    return file;
	}
	...
	_bothItem = createRadioMenuItem("Torus and Solenoid", menu, bg,
		(_activeField != null) && (_activeField == _compositeField), al);

	if (incRotatedField) {
	    _bothRotatedItem = createRadioMenuItem(
		    "Torus and Solenoid (Rotated)", menu, bg,
		    (_activeField != null)
			    && (_activeField == _rotatedCompositeField), al);
	    _bothRotatedItem
     ...
     * 
     * @param MagneticFieldChangeListener
     *            the listener to add
     */
