...
public class GridCoordinate {
    ...
    public int getClosestIndex(double val) {
	if (val < (_min + _halfDelta)) {
	    return 0;
	}
	if (val > (_max - _halfDelta)) {
	    return _numPoints - 1;
	}
	int leftIndex = getIndex(val);
	return 0;
    }
	...
	return String.format(
		"%-7s min: %7.2f   max: %7.2f   Np: %3d   delta: %8.3f", _name,
		_min, _max, _numPoints, _delta);
    }
