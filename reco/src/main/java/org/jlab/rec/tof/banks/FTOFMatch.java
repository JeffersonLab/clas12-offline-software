...
public class FTOFMatch implements IMatchedHit{
			...
			for(int i =0; i< hitlists.size(); i++) {
				if(hitlists.get(i).size()>0) {
					...
					for(BaseHit h : hitlists.get(i)) {
						...
						matchLists.add(hit);
						System.out.println(i+")  s "+h.get_Sector()+" l "+h.get_Layer()+" c "+h.get_Component()+" adcL "+h.get_ADC1()+" adcR "+h.get_ADC2()+" tdcL "+
								h.get_TDC1()+" tdcR "+h.get_TDC2());
					}
				}
			}
	...
	public static void main (String arg[]) throws IOException{
		//String inputFile = "/Users/ziegler/Workdir/Files/HIPO/clasdispr_101.hipo";
