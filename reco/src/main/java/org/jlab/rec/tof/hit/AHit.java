...
public abstract class AHit implements Comparable<AHit>{
	...
	private double  _Energy1Unc;		// (FTOF) L (CTOF) U deposited energy uncertainty
	private double  _Energy2;			// (FTOF) R (CTOF) D deposited energy
	private double  _Energy2Unc;		// (FTOF) R (CTOF) D deposited energy uncertainty
	...
	private double  _lambda1Unc;		// uncertainty in attenuation length ((FTOF) L (CTOF) U)
	private double  _lambda2Unc;		// uncertainty in attenuation length ((FTOF) R (CTOF) D)
	private String 	_StatusWord;		// a status word:  1111 - fully functioning, 0111-noADC1, 1011-noTDC1, 1101-noADC2, 1110-noTDC2((FTOF) L=1,R=2 (CTOF) U=1,D=2)
	private int 	_AssociatedClusterID = -1;
	private double   AdcToEConv; 		// conversion factor
	private double  _y = Double.NaN; 	// hit coordinate measured with respect to the center of the scintillator bar; init as undefined
	private double  _yUnc;				// hit coordinate uncertainty [with respect to the center of the scintillator bar]
	private Point3D _TrkPosition;		// track hit position measured from the reconstructed track info in lab frame
	private double  _yTrk;				// hit coordinate measured with respect to the center of the scintillator bar from the reconstructed track
	 ...
	 * @param ScinBarThickn
	 */
	public void set_HitParams(int superlayer, 
			double TW01, double TW02, double TW11, double TW12,
			double lambda1, double lambda2, double yOffset, 
			double v1, double v2, double v1Unc, double v2Unc,
			double PED1, double PED2, double PED1Unc, double PED2Unc,
			double paddle2paddle, double timeOffset, double LSBConv, double LSBConvErr,
			double ADC1Err, double ADC2Err, double TDC1Err, double TDC2Err, 
			double ADC_MIP, double ADC_MIPErr, double DEDX_MIP, double ScinBarThickn, double pl) {
		
		
		// the order of calculation matters depending on the status
		String status = this.get_StatusWord();
		// initializing the values:		
		double y = Double.NaN;  // y = hit coordinate measured with respect to the center of the scintillation bar (cm)
		double[] t12 = new double[2];  double[] tErr12 = new double[2];  double[] eDep12 = new double[2]; double[] ePMTErr12 = new double[2]; double tErr = 0;  double eErr = 0; double expFac1 = 0; double expFac2 = 0;
		switch(status) {
			
			case "1111":				// Good: TDC1, TDC2, ADC1, ADC2			
				// 1. Compute time-walk corrections:
				this.set_timeWalk1(this.calc_timeWalk(TW01, TW11, (int) (this.get_ADC1() - PED1))); 
				this.set_timeWalk2(this.calc_timeWalk(TW02, TW12, (int) (this.get_ADC2() - PED2)));
				...
				this.set_yUnc(this.calc_yUnc(status, v1, v2, v1Unc, v2Unc, ADC1Err, ADC2Err));				
				// 4. Compute average hit time:
				//this.set_t( 0.5*(this.get_t1() - y/v1 + this.get_t2() + y/v2));  	// average of times	
				this.set_t( 0.5*(this.get_t1() - (pl/2+y)/v1  + this.get_t2() - (pl/2-y)/v2));
				// 4.1 Both TDCs --> Getting the uncertainty contribution from each
				tErr = this.calc_tErr(this.get_y(), this.get_yUnc(), this.get_t1(), this.get_t1Unc(), v1, v1Unc, this.get_t2(), this.get_t2Unc(), v2, v2Unc, pl);
				this.set_tUnc(tErr);
				// 5. Compute energy 1 and 2 & associated uncertainties:
				eDep12 = this.calc_Edep12(ADC_MIP, DEDX_MIP, ScinBarThickn, PED1, PED2);
				this.set_Energy1(eDep12[0]);
				this.set_Energy2(eDep12[1]);
				ePMTErr12 = this.calc_PMTErr12(ADC_MIP, ADC_MIPErr, DEDX_MIP, ScinBarThickn, PED1Unc, PED2Unc, ADC1Err, ADC2Err);
				...
				this.set_Energy(Math.sqrt(eDep12[0]*expFac1*eDep12[1]*expFac2)); 
				// 6.1 Both ADCs 
				eErr = this.calc_EdepErr(this.get_Energy(), this.get_y(), this.get_yUnc(),this.get_Energy1(), this.get_Energy1Unc(), this.get_lambda1(), this.get_lambda1Unc(), this.get_Energy2(), this.get_Energy2Unc(), this.get_lambda2(), this.get_lambda2Unc());				 
				this.set_EnergyUnc(eErr);
				
				break;
			
			case "0111": case "1101":		// Good: TDC1, TDC2  Missing: ADC1 or ADC2
				...
				this.setMissingADC(y, PED1, PED2);
		 		// 3. Compute time-walk corrections (with the estimated ADC1,2 value):
				this.set_timeWalk1(this.calc_timeWalk(TW01, TW11, (int) (this.get_ADC1() - PED1))); 
				this.set_timeWalk2(this.calc_timeWalk(TW02, TW12, (int) (this.get_ADC2() - PED2)));		
				...
				if(Character.toString(this.get_StatusWord().charAt(0)).equals("1"))	{	// ADC1 is valid
					this.set_Energy1Unc(ePMTErr12[0]); //  PMT1 energy uncertainty
					expFac1 = Math.exp(this.get_y()/this.get_lambda1());		
					this.set_Energy(eDep12[0]*expFac1); 
					eErr = this.calc_EdepErr(this.get_Energy(), this.get_y(), this.get_yUnc(),this.get_Energy1(), this.get_Energy1Unc(), this.get_lambda1(), this.get_lambda1Unc(), 0, 0, 0, 0);				 
					this.set_EnergyUnc(eErr);
				}
				if(Character.toString(this.get_StatusWord().charAt(2)).equals("1"))	{	// ADC2 is valid
					this.set_Energy2Unc(ePMTErr12[1]); //  PMT2 energy uncertainty	
					expFac2 = Math.exp(-this.get_y()/this.get_lambda2());		 
					this.set_Energy(eDep12[1]*expFac2);
					eErr = this.calc_EdepErr(this.get_Energy(), this.get_y(), this.get_yUnc(),0, 0, 0, 0, this.get_Energy2(), this.get_Energy2Unc(), this.get_lambda2(), this.get_lambda2Unc());				 
					this.set_EnergyUnc(eErr);
				}
			...
			case "1011": case "1110":		// Good: ADC1, ADC2  Missing: TDC1 or TDC2
				// 1. Compute time-walk corrections:
				this.set_timeWalk1(this.calc_timeWalk(TW01, TW11, (int) (this.get_ADC1() - PED1))); 
				this.set_timeWalk2(this.calc_timeWalk(TW02, TW12, (int) (this.get_ADC2() - PED2)));		

				// 2. Determine hit coordinate from tracking or from ADC log ratio if tracking is missing
				if(! Double.isNaN(this.get_yTrk()))  {	// use tracking info
	        		y = this.get_yTrk(); 
	        		this.set_y(y);						
					...
					y = -(this.get_lambda1()*this.get_lambda2()/(this.get_lambda1()+this.get_lambda2()))*Math.log((double) (this.get_ADC1()-PED1)/(double) (this.get_ADC2()-PED2)) ;
					this.set_y(y);						
				...
				this.set_t1(t12[0]);
				this.set_t2(t12[1]);
				if(Character.toString(this.get_StatusWord().charAt(1)).equals("1"))		// TDC1 is valid
					this.set_t(this.get_t1() - (pl/2 + y)/v1 - yOffset/2.);
				if(Character.toString(this.get_StatusWord().charAt(3)).equals("1"))		// TDC2 is valid
					this.set_t(this.get_t2() + (y - pl/2)/v2 + yOffset/2.);					
				tErr12 = this.calc_tErr12(TDC1Err, TDC2Err, LSBConv, LSBConvErr);
				this.set_t1Unc(tErr12[0]);
				this.set_t2Unc(tErr12[1]);
				if(Character.toString(this.get_StatusWord().charAt(1)).equals("1"))		// TDC1 is valid
					this.set_tUnc(this.calc_tErr(this.get_y(), this.get_yUnc(), this.get_t1(), this.get_t1Unc(), v1, v1Unc, 0, 0, 0, 0, pl));
				if(Character.toString(this.get_StatusWord().charAt(3)).equals("1"))		// TDC2 is valid
					this.set_tUnc(this.calc_tErr(this.get_y(), this.get_yUnc(), 0, 0, 0, 0, this.get_t2(), this.get_t2Unc(), v2, v2Unc, pl));
			...
			case "1100": case "0011": case "0110": case "1001":	//Good: TDC1, ADC1  Missing: TDC2, ADC2 or Good: TDC2, ADC2  Missing: TDC1, ADC1 or Good: TDC1, ADC2  Missing: TDC2, ADC1 or Good: TDC2, ADC1  Missing: TDC1, ADC2
	 ...
	 * @return uncertainty in y for cases 1) TDC1, TDC2, ADC1, ADC2	 are all good, 2) Good: ADC1, ADC2  Missing: TDC1 or TDC2
	 */
	private double calc_yUnc(String status, double v1, double v2, double v1Unc, double v2Unc, double ADC1Err, double ADC2Err) {
		double err = 0;
		...
		switch(status) {
			...
			case "1011": case "1110":		// Good: ADC1, ADC2  Missing: TDC1 or TDC2 
				...
				double jterm1 	= ((-l2/(l1+l2))*(-l2/(l1+l2))*Math.log(ADC1/ADC2))*l1Err;
				double jterm1Sq	= jterm1*jterm1;
				double jterm2 	= ((-l1/(l1+l2))*(-l1/(l1+l2))*Math.log(ADC1/ADC2))*l2Err;
	...
	private void setMissingADC(double y, double PED1, double PED2) {
		// if 1 ADC is missing
		if(Character.toString(this.get_StatusWord().charAt(0)).equals("0")) {  // ADC1 is missing ==> ADC1 = ADC2 *  exp(-y/Lambda1 -y/Lambda2)
	 		int ADC1 = (int) ((this.get_ADC2() - PED2)*Math.exp(-y/this.get_lambda1()-y/this.get_lambda2())); 
	 		this.set_ADC1(ADC1); // now that the left adc is set the timewalk will be calculated using this value
	 	} 
	 	if(Character.toString(this.get_StatusWord().charAt(2)).equals("0")) { // ADC2 is missing ==> ADC2 = ADC1 *  exp(y/Lambda1 + y/Lambda2)
	 		int ADC2 = (int) ((this.get_ADC1() - PED1)*Math.exp(y/this.get_lambda1()+y/this.get_lambda2())); 
	 		this.set_ADC2(ADC2); // now that the right adc is set the timewalk will be calculated using this value
	 	}
	}
	 ...
	 * @return the uncertainty of the missing ADC calculated using the track information
	 */
	...
	private double calc_missingADCUnc(double ADC1Err, double ADC2Err) {
		...
		if(Character.toString(this.get_StatusWord().charAt(0)).equals("0")) { // ADC1 is missing ==> ADC1 = ADC2 *  exp(-y/Lambda1 -y/Lambda2)
	 		double expVal = Math.exp(-y/this.get_lambda1()-y/this.get_lambda2()); 
	 		double ADCErr = ADC2Err;
	 		double term1Sq = ADCErr*ADCErr;
	 		double term2Sq = (this.get_ADC2()*yErr/this.get_lambda1()+yErr/this.get_lambda2())*(this.get_ADC2()*yErr/this.get_lambda1()+yErr/this.get_lambda2());
	 		double term3Sq = (this.get_ADC2()*y*this.get_lambda1Unc()/(this.get_lambda1()*this.get_lambda1()))*(this.get_ADC2()*y*this.get_lambda1Unc()/(this.get_lambda1()*this.get_lambda1()))
	 				       + (this.get_ADC2()*y*this.get_lambda2Unc()/(this.get_lambda2()*this.get_lambda2()))*(this.get_ADC2()*y*this.get_lambda2Unc()/(this.get_lambda2()*this.get_lambda2()));
	 		err = expVal*Math.sqrt(term1Sq+term2Sq+term3Sq);
	 	}
	 	if(Character.toString(this.get_StatusWord().charAt(2)).equals("0")) { // ADC2 is missing ==> ADC1R = ADC1 *  exp(y/Lambda1 + y/Lambda2)
	 		double expVal = (this.get_ADC1()*Math.exp(y/this.get_lambda1()+y/this.get_lambda2())); 
	 		double ADCErr = ADC1Err;
	 		double term1Sq = ADCErr*ADCErr;
	 		double term2Sq = (this.get_ADC1()*yErr/this.get_lambda1()+yErr/this.get_lambda2())*(this.get_ADC1()*yErr/this.get_lambda1()+yErr/this.get_lambda2());
	 		double term3Sq = (this.get_ADC1()*y*this.get_lambda1Unc()/(this.get_lambda1()*this.get_lambda1()))*(this.get_ADC1()*y*this.get_lambda1Unc()/(this.get_lambda1()*this.get_lambda1())) 
				           + (this.get_ADC1()*y*this.get_lambda2Unc()/(this.get_lambda2()*this.get_lambda2()))*(this.get_ADC1()*y*this.get_lambda2Unc()/(this.get_lambda2()*this.get_lambda2()));
	 		err = expVal*Math.sqrt(term1Sq+term2Sq+term3Sq);
	 	}
	 	return err;
	}
	 ...
	 * @return double[] index 0 = t1, index 1 = t2, the reconstructed time using 1,2 TDCs
	 */
	private double[] calc_t12(double paddle2paddle, double timeOffset, double LSBConv){
		double[] t12 = new double[2];
		
		//check status of TDCs and then calc t12 --> if TDC is not valid, set t to zero
		double t1 = 0;
		double t2 = 0;
		if(Character.toString(this.get_StatusWord().charAt(1)).equals("1"))  // TDC1 is valid
			t1 = this.get_TDC1()*LSBConv -timeOffset/2. -this.get_timeWalk1() + paddle2paddle;
		if(Character.toString(this.get_StatusWord().charAt(3)).equals("1"))  // TDC2 is valid
			t2 = this.get_TDC2()*LSBConv +timeOffset/2. -this.get_timeWalk2() + paddle2paddle;
	 ...
	 */
	private double[] calc_tErr12(double TDC1Err, double TDC2Err, double LSBConv, double LSBConvErr){
		...
		double tErr2 = 0;
		if(Character.toString(this.get_StatusWord().charAt(1)).equals("1"))  // TDC1 is valid
			tErr1 = Math.sqrt( (this.get_TDC1()*LSBConvErr)*(this.get_TDC1()*LSBConvErr) 
			+ (TDC1Err*LSBConv)*(TDC1Err*LSBConv) );
		if(Character.toString(this.get_StatusWord().charAt(3)).equals("1"))  // TDC2 is valid
			tErr2 = Math.sqrt( (this.get_TDC2()*LSBConvErr)*(this.get_TDC2()*LSBConvErr) 
			+ (TDC2Err*LSBConv)*(TDC2Err*LSBConv) );
	 ...
	 * @param v1 effective velocity [L for FTOF, U for CTOF]
	 * @param v1Unc uncertainty in the effective velocity [for FTOF vL, for CTOF vU]
	 * @param t2 for FTOF tR, for CTOF tD 
	 * @param t2Unc uncertainty in t2 [for FTOF tR, for CTOF tD]
	 * @param v2 effective velocity [R for FTOF, D for CTOF]
	 * @param v2Unc uncertainty in the effective velocity [for FTOF vR, for CTOF vD]
	 * @param L the paddle length
	 * @return the time uncertainty 
	 */
	private double calc_tErr(double y, double yUnc, double t1, double t1Unc, double v1, double v1Unc, double t2, double t2Unc, double v2, double v2Unc, double L) {
		
		double errSq = 0;
		
		if(t2 ==0 )
			errSq = t1Unc*t1Unc + (yUnc/v1)*(yUnc/v1) + ((y+L/2)*v1Unc/(v1*v1))*((y+L/2)*v1Unc/(v1*v1));
		if(t1 ==0 )
			errSq = t2Unc*t2Unc + (yUnc/v2)*(yUnc/v2) + ((L/2-y)*v2Unc/(v2*v2))*((L/2-y)*v2Unc/(v2*v2));
		if(t1!=0 && t2!=0)
			errSq = (t1Unc*t1Unc + ((y+L/2)*v1Unc/(v1*v1))*((y+L/2)*v1Unc/(v1*v1))+
				  + t2Unc*t2Unc + ((L/2-y)*v2Unc/(v2*v2))*((L/2-y)*v2Unc/(v2*v2))
				  +(yUnc*yUnc*(1/v2 - 1/v1)*(1/v2 - 1/v1)))/4.;
	 ...
	 * @param PED1 for FTOF L pedestal, for CTOF U pedestal 
	 * @param PED2 for FTOF R pedestal, for CTOF D pedestal 
	 * @return double[] index 0 = for ADC1, index 1 = for ADC2 corresponding to the (FTOF) L,R or (CTOF) U, D PMT energy
	 */
	private double[] calc_Edep12(double ADC_MIP, double DEDX_MIP, double ScinBarThickn, double PED1, double PED2) {
	 ...
	 */
	private double[] calc_PMTErr12(double ADCMIP, double ADCMIPErr, double DEDX_MIP, double ScinBarThickn, double PEDErr1, double PEDErr2, double ADCErr1, double ADCErr2) {
		double[] eDepErr12 = new double[2];
		
		double K = DEDX_MIP*ScinBarThickn;
	
		// PMT errors
		if(Character.toString(this.get_StatusWord().charAt(0)).equals("1"))  // ADC1 is OK
			eDepErr12[0] = K*Math.sqrt( (this.get_Energy1()*this.get_Energy1()/(K*K) )*ADCMIPErr*ADCMIPErr + ADCErr1*ADCErr1 + PEDErr1*PEDErr1)/ADCMIP;
	 	if(Character.toString(this.get_StatusWord().charAt(2)).equals("1"))  // ADC2 is OK 
	 		eDepErr12[1] = K*Math.sqrt( (this.get_Energy2()*this.get_Energy2()/(K*K) )*ADCMIPErr*ADCMIPErr + ADCErr2*ADCErr2 + PEDErr2*PEDErr2)/ADCMIP;
	 ...
	 * @return the square of the uncertainty on the deposited energy for [FTOF L, CTOF U] 1 or [FTOF R, CTOF D] 2  
	 */
	private double calc_EdepErr(double E, double y, double yUnc, 
			double E1, double E1Unc, double lambda1, double lambda1Unc, 
			double E2, double E2Unc, double lambda2, double lambda2Unc) {
		...
		if(E2 == 0) {
			...
			double term3Sq = (lambda1Unc/(lambda1*lambda1))*(lambda1Unc/(lambda1*lambda1));		
			Sq = term1Sq+term2Sq+term3Sq;
		}
		if(E1 == 0) {
			double term1Sq = (E2Unc/E2)*(E2Unc/E2);
			double term2Sq = (yUnc/lambda2)*(yUnc/lambda2);
			double term3Sq = (lambda2Unc/(lambda2*lambda2))*(lambda2Unc/(lambda2*lambda2));		
			Sq = term1Sq+term2Sq+term3Sq;
		}
		if(E1 != 0 && E2 !=0 ) {
			double term1Sq = ( (E2Unc/E2)*(E2Unc/E2) + (E1Unc/E1)*(E1Unc/E1) )/4.;
			double term2Sq = ( yUnc*yUnc*(1./lambda1 - 1./lambda2)*(1./lambda1 - 1./lambda2) )/4. ;
			double term3Sq = ( (lambda1Unc/(lambda1*lambda1))*(lambda1Unc/(lambda1*lambda1)) + (lambda2Unc/(lambda2*lambda2))*(lambda2Unc/(lambda2*lambda2)) )/4.;		
			Sq = term1Sq+term2Sq+term3Sq;
		}
	...
	public void printInfo() {
		...
		String s = " FTOF Hit in "+" Sector "+this.get_Sector()+" Panel "+this.get_Panel()+" Paddle "+this.get_Paddle()+" with Status "+this.get_StatusWord()+" in Cluster "+this.get_AssociatedClusterID()+" : \n"+
				...
				"  t2 =  "+form.format(this.get_t2())+ 
				"  t =  "+form.format(this.get_t())+ 
				"  timeWalk1 =  "+form.format(this.get_timeWalk1())+ 
				"  timeWalk2 =  "+form.format(this.get_timeWalk2())+ 
				"  lambda1 =  "+form.format(this.get_lambda1())+ 
				"  lambda2 =  "+form.format(this.get_lambda2())+ 
				"  Energy =  "+form.format(this.get_Energy())+ 
				"  Energy1 =  "+form.format(this.get_Energy1())+ 
				"  Energy2 =  "+form.format(this.get_Energy2())+ 
