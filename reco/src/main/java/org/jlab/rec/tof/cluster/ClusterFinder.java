	...
	*/
	...
	public ArrayList<Cluster> findClusters(List<?> hits2, int nsectors, int npanels, int[] npaddles) {
		...
		if(hits2!=null) { 
			...
			// a Hit Array is used to identify clusters, panel1B is the one with the maximum number of scintillator bars
			...
			for(int i =0; i<hits2.size(); i++) {
				...
				if(s>0 && s<=nsectors && l>0 && l<=npanels &&  w>0 && w<=npaddles[l-1] ) 
				...
				for(int l=0; l<npanels; l++) {		
					...
					while(si<npaddles[l]) {
						...
						if(HitArray[si][l][s] != null) {
							...
							try {
								while(HitArray[si][l][s] != null  && si<npaddles[l]) {  
