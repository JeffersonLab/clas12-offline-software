...
public class Cluster extends ArrayList<AHit> implements Comparable<Cluster>{
	...
	private double[] _tCorr;		// corrected time using 3 algorithms to compute deltaR
	
	private double _y_locUnc;		// uncertainty in y in the local cluster coordinate system 
	private double _EnergyUnc;		// uncertainty in total energy of the cluster	
	private double _tUn;			// uncertainty in cluster time
	
	private String _StatusWord;		// a status word for each hit:  1111 - fully functioning, 0111-noADC[L,U], 1011-noTDC[L,U], 1101-noADC[R,D], 1110-noTDC[R,D]; the cluster word is the status of each hit in the cluster 
	...
	public void calc_Centroids() {
		...
		if(this.size() != 0) {
			...
			double errYlocSq		= 0;		    // contribution factor to uncertainty in local y
			double errTSq 			= 0;			// contribution factor to uncertainty in time
			
			String statusWord 		="";			// the status word
			int StWordBt1 = 0; int StWordBt2 = 0; int StWordBt3 = 0; int StWordBt4 = 0;
			
			for(AHit thehit : this) {
				
				StWordBt1 += Character.getNumericValue(thehit.get_StatusWord().charAt(0));
				StWordBt2 += Character.getNumericValue(thehit.get_StatusWord().charAt(1));
				StWordBt3 += Character.getNumericValue(thehit.get_StatusWord().charAt(2));
				StWordBt4 += Character.getNumericValue(thehit.get_StatusWord().charAt(3));
				// Centroid terms in global coordinate system.  The values are discretized in x and z since the measured values in the local frame are the center of the bar in local x and the position along the bar in local y
				...
				errYlocSq += E*thehit.get_yUnc()* E*thehit.get_yUnc() + thehit.get_y()*thehit.get_EnergyUnc()* thehit.get_y()*thehit.get_EnergyUnc();
				
				errTSq+= E*thehit.get_tUnc()* E*thehit.get_tUnc() + thehit.get_t()*thehit.get_EnergyUnc()* thehit.get_t()*thehit.get_EnergyUnc();
	...
	public void printInfo() {
		...
		String s = "Cluster in "+" Sector "+this.get_Sector()+" Panel "+this.get_Panel()+" ID "+this._Id+" : \n"+
				" t =  "+form.format(_t)+ 
				" Energy =  "+form.format(_Energy)+ 
				" xPos =  "+form.format(_x)+
				" yPos =  "+form.format(_y)+
				" zPos =  "+form.format(_z);
