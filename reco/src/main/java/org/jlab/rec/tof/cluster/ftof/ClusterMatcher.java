...
public class ClusterMatcher {
	...
	private double[] _pathLen = new double[3];	// pathlength of track between counters for 3 cases(taking earlier hit time, max energy or average hit times)
	public  double Beta = 0;					// beta of the track to be obtained from the event builder
	private double _xMatch = Double.NaN;		// matched x coordinate  between panel 1a and panel 1b
	private double _yMatch = Double.NaN;		// matched y coordinate  between panel 1a and panel 1b
	private double _zMatch = Double.NaN;		// matched z coordinate  between panel 1a and panel 1b
	private double _tMatch = Double.NaN;		// matched time  between panel 1a and panel 1b
	...
	public ArrayList<Cluster> ClusterDoublet(Cluster C1a, Cluster C1b) { // CHECK this...
		...
		// computes an array of pathlengths for (cluster size=1) 1 or (cluster size=2) 3 reference points along 
		...
		if(Double.isNaN(_xMatch) || Double.isNaN(_yMatch) || Double.isNaN(_zMatch) || Double.isNaN(_tMatch))
			return null;		// not matched
	...
	private double calc_deltaT(int panel, int paddle) {
		...
		double delta_t = Constants.DELTA_T[panel-1][0]*paddle + Constants.DELTA_T[panel-1][1];
	...
	private double[] get_CorrectedHitTime(Cluster clus1A, Cluster clus1B) {
		...
		if(clus1A!=null && clus1B!=null && Beta>0)  { // clusters and pathlength have to be well defined
			double delta_t1a 	= this.calc_deltaT(clus1A.get_Panel(), CntrInPan1aWithEmax);
			double delta_t1b 	= this.calc_deltaT(clus1A.get_Panel(), CntrInPan1bWithEmax);
			
			double term1 = clus1B.get_t()/delta_t1b;
			double term3 = 1./delta_t1a + 1./delta_t1b;
			for(int i = 0; i<3; i++) {
				if(this._pathLen[i]>0) {
					term2[i] = (clus1A.get_t()-this._pathLen[i]/Beta)/delta_t1a;
