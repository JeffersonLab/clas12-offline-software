...
public class BSTGeometry {
     ...
     * @return
     */
//    public static Point3D inverseRotate(int sector, int superlayer, int layer, Point3D p3d) {
//	    Transformation3D t3d = _transform.get(sector, superlayer, layer).inverse();
...
//    }
     ...
     * @param sector the 1-based layer dependent sector
     * @param layer the "big" layer 1..8
     * @param coords (dim = 26) will hold quads as [x1, y1, z1, ... x4, y4, z4]
     *            for quad 1 (12 numbers) [x1, y1, z1, ... x4, y4, z4] for quad
     *            2 (12 numbers) [x1, y1, z1, ... x4, y4, z4] for quad 3 (12
     *            numbers)
     */
    ...
    private static void fillCoords(int index, float coords[], float x1,
	    float y1, float x2, float y2, float zmin, float zmax) {
     ...
     * @param vals holds for (10) numbers. All are in mm. The first four are x,
	...
	for (int strip = 1; strip <= 256; strip++) {
	    ...
	    String s = String.format(
		    "strip %4d  coords: [%-9.3f  %-9.3f  %-9.3f]  [%-9.3f  %-9.3f  %-9.3f]  ",
		    strip, coords[0], coords[1], coords[2], coords[3],
		    coords[4], coords[5]);
	    System.err.println(s);
	}
	...
	System.out.println("done.");
	 String s = String.format("%-9.3f %-9.3f %-9.3f %-9.3f %-9.3f %-9.3f %-9.3f %-9.3f %-9.3f %-9.3f",
	 vals[0],
	 vals[1], vals[2], vals[3], vals[4],
	 vals[5], vals[6], vals[7], vals[8], vals[9]);
	 System.err.println(s);
