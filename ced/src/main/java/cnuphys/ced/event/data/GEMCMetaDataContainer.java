...
public class GEMCMetaDataContainer implements IClasIoEventListener  {
	...
	public GEMCMetaDataContainer(ClasIoEventManager eventManager) {
		eventManager.addClasIoEventListener(this, 0);
	}
		...
		if (node != null) {
			...
			if (bytes != null) {
				...
				if (ss != null) {
					...
					Ced.getCed().getGEMCView().getGEMCTable()
							.getGEMCMetaDataModel().setData(properties);
				} // ss not null
			} // bytes not null
		} // node not null
