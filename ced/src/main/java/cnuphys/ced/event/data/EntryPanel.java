...
public class EntryPanel extends JPanel {
	...
	public EntryPanel(ExpressionPanel ePanel) {
		...
		add(_expressionText, BorderLayout.CENTER);
	}
	
	private void addNorth() {
		// the functions
		JTextArea fdef = new JTextArea("", 4, 40);
		fdef.setOpaque(true);
	...
	//	fdef.setForeground(Color.cyan);
		fdef.setText("abs, acos, asin, atan, atan2, cbrt, ceil, cos, cosh, exp, expm1, floor, log,\n"
				...
				"Prepend named variables with an underscore, e.g., atan2(_x, _y)");

		fdef.setBorder(new CommonBorder("Allowed functions (see Java Math class for usage)"));
