...
public class Geometry {
	...
	public double CRZ_GetAngleStrip(int sector, int layer, int strip) {
	...
	public double CRC_GetZStrip(int sector, int layer, int strip) {
	...
	public static void main(String arg[]) {
		...
		for (int sector = 1; sector <=3; sector++) {
			for (int layer = 5; layer <= 6; layer++) {
				double beginAng = Math.toDegrees(geo.CRC_GetBeginStrip(sector, layer));
				double endAng = Math.toDegrees(geo.CRC_GetEndStrip(sector, layer));
				double delAng = endAng-beginAng;
				if (delAng < 0) delAng += 360;
				System.out.println("sector: " + sector + "  layer: " + layer + "   begin: " + beginAng+ "   end: " + endAng + "  delAng: " +
						delAng);
			}
		}
