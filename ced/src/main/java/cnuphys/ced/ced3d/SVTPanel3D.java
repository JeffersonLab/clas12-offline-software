...
public class SVTPanel3D extends DetectorItem3D {
	...
	if (showCosmics() && (ids != null)) {
		...
		Support3D.drawLine(drawable, (float)x1, (float)y1, (float)z1, (float)x2, (float)y2, (float)z2, 
			cosmicColor, 1.5f);
	...
	if (showCrosses() && (bstData.bstrec_crosses_x != null)) {
		...
		Support3D.drawLine(drawable, x1, y1, z1, (float) (ux[i]),
			(float) (uy[i]), (float) (uz[i]), CROSS_LEN, crossColor, 3f);
		Support3D.drawLine(drawable, x1, y1, z1, (float) (ux[i]),
			(float) (uy[i]), (float) (uz[i]), (float) (1.1*CROSS_LEN), Color.black, 1f);
...
//		
////		System.err.println("BST crosses at (" + x1 + ", " + y1 + ", " + z1 + ")");
//		
//		Support3D.drawCone(drawable, x1, y1, z1, x2, y2, z2, 1.0f, coneColor);
//
...
//			x2, y2, z2, coneLineColor, 2f);
	...
	if (showme) {
	    switch (_layer) {
	    case 1:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_1);
		break;
	    case 2:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_2);
		break;
	    case 3:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_3);
		break;
	    case 4:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_4);
		break;
	    case 5:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_5);
		break;
	    case 6:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_6);
		break;
	    case 7:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_7);
		break;
	    case 8:
		showme = ((CentralPanel3D) _panel3D).show(CedPanel3D.SHOW_SVT_LAYER_8);
		break;
	    }
	}
