...
public class ReconDrawer extends SectorViewDrawer {
	...
	if (_view.showDChbHits()) {
	    drawDCReconHits(g, container, dcData, 0);
	}

	// drift chamber time based based
	if (_view.showDCtbHits()) {
	    drawDCReconHits(g, container, dcData, 1);
	}
    ...
    private void drawDCReconHits(Graphics g, IContainer container,
	    DCDataContainer dcData, int option) {
	...
	if (option == 0) {
	    ...
	    color = DataDrawSupport.DC_HB_COLOR;
	}
	    ...
	    color = DataDrawSupport.DC_TB_COLOR;
	}
	else {
	    return;
	}
	...
	if ((hbHitCount > 0) && (wire != null) && (layer != null)
		...
		if (_view.isSectorOnView(sector[hitIndex])) {
			...
			//draw a diagonal line indicating this wire is a recon hit
			//tb and/or hb
			...
			String s = fbcolors[option] + prefix[option]
