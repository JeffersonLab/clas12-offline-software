...
public class FTCalXYPolygon extends Polygon {
 ...
 //	System.err.println("PADDLE ID: " + paddleId + "   good paddle: " + (paddle != null));
     }
 	...
 	Point pp = new Point();
 	for (int i = 0; i < 4; i++) {
 	    container.worldToLocal(pp, paddle.getVolumePoint(i).x(),
 		    paddle.getVolumePoint(i).y());
 	    addPoint(pp.x, pp.y);
 	}
