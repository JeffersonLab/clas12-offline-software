...
public class PCALHexSectorItem extends HexSectorItem {
	...
	for (int stripType = 0; stripType < 3; stripType++) {
		...
		for (int stripIndex = 0; stripIndex < PCALGeometry.PCAL_NUMSTRIP[stripType]; stripIndex++) {
		    _stripPoly[stripType][stripIndex] = stripPolygon(stripType,
			    stripIndex);
	...
	ECDataContainer ecData = _eventManager.getECData();
	for (int i = 0; i < ecData.getHitCount(ECDataContainer.PCAL_OPTION); i++) {
	...
	Point pp = new Point();
	_pcalView.ijkToScreen(_sector, orig, pp);
     ...
     * 
     * @param stripType
     *            PCAL_U, PCAL_V, or PCAL_W [0..2]
     * @param stripIndex
     *            the strip index [0..(EC_NUMSTRIP-1)]
     * @return
     */
    public Polygon stripPolygon(int stripType, int stripIndex) {
	Polygon poly = new Polygon();
	Point pp = new Point();

	for (int i = 0; i < 4; i++) {
	    Point3D pijk = PCALGeometry.getStripPoint(stripType, stripIndex, i);
	    _pcalView.ijkToScreen(getSector(), pijk, pp);
	...
	for (int i = 0; i < 3; i++) {
	    Point3D pijk = PCALGeometry.getTrianglePoint(i);
	    _pcalView.ijkToScreen(getSector(), pijk, pp);
	    poly.addPoint(pp.x, pp.y);
	}
