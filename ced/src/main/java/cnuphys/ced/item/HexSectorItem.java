...
public abstract class HexSectorItem extends PolygonItem {
	...
	Point p1 = new Point();
	Point p2 = new Point();
	sector2DToLocal(p1, sp1);
	sector2DToLocal(p2, sp2);
     ...
     */
    public void sector2DToLocal(Point pp, Point2D.Double sp) {
     ...
     *            will hold the sector coordinates
     */
    public void localToSector2D(Point pp, Point2D.Double sp) {
	Point2D.Double wp = new Point2D.Double();
	getContainer().localToWorld(pp, wp);
	// world are same as lab
	sector2DToLab2D(sp, wp);
    }
