...
public class LundTrackDialog extends JDialog {
	...
	if (swimmer != null) {
		...
		try {
			...
			//
			// //the new swimmer
			// SwimZStateVector start = new SwimZStateVector(xo, yo,
			// zo, p, theta, phi);
			//
			...
			//
			// SwimZ sz = new
			// SwimZ(MagneticFields.getActiveField());
			// long startTime = System.currentTimeMillis();
			// for (int i = 0; i < numTimes; i++) {
			// try {
			// result = sz.adaptiveRK(Q, p, start, zf,
			...
			// }
			// double timePerSwim =
			// ((double)(System.currentTimeMillis()-startTime))/numTimes;
