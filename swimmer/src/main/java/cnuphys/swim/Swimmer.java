...
public class Swimmer {
     ...
     */
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IStopper stopper,
	    double maxPathLength, double stepSize, double distanceBetweenSaves) {
	...
	if ((_field == null) || (charge == 0)) {
	    GeneratedParticleRecord genPartRec = new GeneratedParticleRecord(
	...
	SwimTrajectory trajectory = new SwimTrajectory(charge, xo, yo, zo,
		momentum, theta, phi, nsave, 100);

	// Integrate
	DefaultDerivative deriv = new DefaultDerivative(charge, momentum,
		_field);
	ntotal = (new RungeKutta4()).uniformStep(uo, 0, maxPathLength, u, s,
		deriv, stopper);

	// now cycle through and get the save points
	for (int i = 0; i < ntotal; i++) {
	    if (((i % cycle) == 0) || (i == (ntotal - 1))) {
		double v[] = makeVector(u[0][i], u[1][i], u[2][i], u[3][i],
			u[4][i], u[5][i]);
		trajectory.add(v);
	    }
	}
     ...
     */
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, final double rmax,
	    double maxPathLength, double stepSize, double distanceBetweenSaves) {

	IStopper stopper = new DefaultSwimStopper(rmax);

	return swim(charge, xo, yo, zo, momentum, theta, phi, stopper,
     ...
     */
    public int swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IStopper stopper,
	    IRk4Listener listener, double maxPathLength, double stepSize) {
	...
	DefaultDerivative deriv = new DefaultDerivative(charge, momentum,
		_field);
	return (new RungeKutta4()).uniformStep(uo, 0, maxPathLength, stepSize,
		deriv, stopper, listener);
    }
     ...
     */
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, final double fixedZ,
	    final double accuracy, double maxR, double maxPathLength,
	    double stepSize, double distanceBetweenSaves) {
	...
	// our first attempt
	SwimTrajectory trajectory = swim(charge, xo, yo, zo, momentum, theta,
		phi, stopper, maxPathLength, stepSize, distanceBetweenSaves);
	...
	while ((count < maxtry) && (del > accuracy)) {
	    ...
	    SwimTrajectory addTraj = swim(charge, xo, yo, zo, momentum, theta,
		    phi, stopper, maxPathLength, stepSize, distanceBetweenSaves);
    ...
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, final double fixedZ,
	    double accuracy, double maxR, double maxPathLength,
	    double stepSize, double relTolerance[], double hdata[])
	    throws RungeKuttaException {
	...
	traj = swim(charge, xo, yo, zo, momentum, theta, phi, stopper,
		maxPathLength, stepSize, relTolerance, hdata);
	...
	while ((count < maxtry) && (del > accuracy)) {
	    ...
	    SwimTrajectory addTraj = swim(charge, xo, yo, zo, momentum, theta,
		    phi, stopper, maxPathLength, stepSize, relTolerance, hdata);
    ...
    private DefaultSwimStopper zStopper(double maxR, final double fixedZ,
	    final boolean normalDirection) {
    ...
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IStopper stopper,
	    double maxPathLength, double stepSize, double relTolerance[],
	...
	// integrate
	(new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength, stepSize, s, u,
		deriv, stopper, _defaultTableau, relTolerance, hdata);
	// now cycle through and get the save points
	...
	int nstep = (new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength,
		stepSize, deriv, stopper, listener, _defaultTableau,
		relTolerance, hdata);
    ...
    public SwimTrajectory swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IStopper stopper,
	    double maxPathLength, double stepSize, double tolerance,
	    double hdata[]) throws RungeKuttaException {
	...
	// integrate
	(new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength, stepSize, t, y,
		deriv, stopper, _defaultTableau, tolerance, yscale, hdata);
	// now cycle through and get the save points
    ...
    public int swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IStopper stopper,
	    IRk4Listener listener, double maxPathLength, double stepSize,
	    double tolerance, double hdata[]) throws RungeKuttaException {
	...
	int nstep = (new RungeKutta4()).adaptiveStep(uo, 0, maxPathLength,
		stepSize, deriv, stopper, listener, _defaultTableau, tolerance,
		yscale, hdata);
     ...
     */
    public int swim(int charge, double xo, double yo, double zo,
	    double momentum, double theta, double phi, IRk4Listener listener,
	    double rmax, double maxPathLength, double stepSize) {

	IStopper stopper = new DefaultSwimStopper(rmax);

	return swim(charge, xo, yo, zo, momentum, theta, phi, stopper,
		listener, maxPathLength, stepSize);
    }

    // create a straight line trajectory with just two points
    private SwimTrajectory straightLineTrajectory(
	    GeneratedParticleRecord genPartRec, double pathLen) {
	...
	double uf[] = makeVector(xo + delx, yo + dely, zo + delz, uo[3], uo[4],
		uo[5]);
    ...
    private SwimTrajectory straightLineTrajectoryFixedZ(
	    GeneratedParticleRecord genPartRec, double zf) {
     ...
     */
    public static SwimTrajectory swimBackwardsToVertex(int q, double xo,
	    double yo, double zo, double px, double py, double pz) {
	...
	try {
	    traj = swimmer.swim(q, xo, yo, zo, p, theta, phi, ztarget,
		    accuracy, 10, 10, stepSize, Swimmer.CLAS_Tolerance, hdata);
     ...
     */
    private static double[] intitialState(double xo, double yo, double zo,
    ...
    private double[] makeVector(double x, double y, double z, double vx,
	    double vy, double vz) {
