...
public class SwimZ {
     ...
     */
    public SwimZResult adaptiveRK(int Q, double p, SwimZStateVector start,
	    final double zf, double stepSize, double absError[], double hdata[])
	    throws SwimZException {
	if (start == null) {
	...
	int nStep = 0;
	try {
	    nStep = rk4.adaptiveStepToTf(yo, start.z, zf, stepSize, z, y,
		    deriv, _stopper, absError, hdata);
     ...
     * @throws SwimZException
     */
    public SwimZResult uniformRK4(int Q, double p, SwimZStateVector start,
	    double zf, double stepSize) throws SwimZException {
	if (start == null) {
	    throw new SwimZException("Null starting state vector.");
	}
     ...
     * @throws SwimZException
     */
    public SwimZResult parabolicEstimate(int Q, double p,
	    SwimZStateVector start, double zf, double stepSize)
	    throws SwimZException {
	...
	for (int i = 0; i < swimZrange.getNumStep(); i++) {
	    ...
	    SwimZStateVector v1 = new SwimZStateVector(x1, y1,
		    swimZrange.z(i + 1), tx1, ty1);
	...
	SwimZStateVector v = new SwimZStateVector(x1, y1, zf, start.tx,
		start.ty);
	result.add(v);
	return result;
    }
