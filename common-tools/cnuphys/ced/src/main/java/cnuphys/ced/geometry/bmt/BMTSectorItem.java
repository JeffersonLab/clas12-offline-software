...
public class BMTSectorItem extends DonutItem {
	...
	private static LAYERTYPE[] layerTypes = { LAYERTYPE.C, LAYERTYPE.Z, LAYERTYPE.Z, LAYERTYPE.C, LAYERTYPE.Z,
			LAYERTYPE.C };
	...
	public void getFeedbackStrings(IContainer container, Point screenPoint, Point2D.Double wp,
		...
		if (contains(container, screenPoint)) {
			...
			feedbackStrings.add("$lawn green$" + microMegasStr + " sector " + _sector + " = "
					+ _sectorNames[_sector - 1] + " layer " + _layer + " strip " + ((zstrip < 0) ? " N/A " : zstrip));
