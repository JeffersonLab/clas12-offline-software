...
public class Cross2 {
	...
	public Cross2(int sector, int region, int id, float x, float y, float z, float ux, float uy, float uz, float err_x,
			float err_y, float err_z) {
		super();
		this.sector = sector;
		this.region = region;
		this.id = id;
	...
	public boolean isErrorBad() {
		return Float.isNaN(err_x) || Float.isNaN(err_y) || Float.isNaN(err_z);
	}
