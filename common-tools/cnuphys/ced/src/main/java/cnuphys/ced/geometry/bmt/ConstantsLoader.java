...
//		double[] CRCXPOS = new double[NREGIONS]; 		// Distance on the PCB between the PCB first edge and the edge of the first strip in mm
		...
		for (int i = 0; i < dbprovider.length("/geometry/cvt/mvt/bmt_layer/Layer"); i++) {
			...
			// double[] EDGE1 = new
			// double[]{Math.toRadians(Phi_min),Math.toRadians(Phi_min+240),Math.toRadians(Phi_min+120)};
			// double[] EDGE2 = new
			// double[]{Math.toRadians(Phi_max),Math.toRadians(Phi_max-120),Math.toRadians(Phi_max+120)};
			// sector clocking fix
			double[] EDGE1 = new double[] { Math.toRadians(Phi_min + 120), Math.toRadians(Phi_min),
					Math.toRadians(Phi_min + 240) };
			double[] EDGE2 = new double[] { Math.toRadians(Phi_max + 120), Math.toRadians(Phi_max),
					Math.toRadians(Phi_max - 120) };
			spacing = 0;
