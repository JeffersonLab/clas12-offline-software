...
public abstract class AMcHitDrawer extends CedViewDrawer {
	...
	protected void drawGemCXYZHits_HTCC(Graphics g, IContainer container) {
		showGemcXYZHits(g, container, FeedbackRect.Dtype.HTCC, HTCC.avgX(), HTCC.avgY(), HTCC.avgZ(), 0);
	}
...
//				0);
	...
	protected void drawGemCXYZHits_FTOF(Graphics g, IContainer container) {
		showGemcXYZHits(g, container, FeedbackRect.Dtype.FTOF, FTOF.getInstance().avgX(FTOF.PANEL_1A),
				FTOF.getInstance().avgY(FTOF.PANEL_1A), FTOF.getInstance().avgZ(FTOF.PANEL_1A), FTOF.PANEL_1A);

		showGemcXYZHits(g, container, FeedbackRect.Dtype.FTOF, ColumnData.getDoubleArray("FTOF1B::true.avgX"),
				ColumnData.getDoubleArray("FTOF1B::true.avgY"), ColumnData.getDoubleArray("FTOF1B::true.avgZ"),
				FTOF.PANEL_1B);

		showGemcXYZHits(g, container, FeedbackRect.Dtype.FTOF, ColumnData.getDoubleArray("FTOF2B::true.avgX"),
				ColumnData.getDoubleArray("FTOF2B::true.avgY"), ColumnData.getDoubleArray("FTOF2B::true.avgZ"),
				FTOF.PANEL_2);
	}

	protected void showGemcXYZHits(Graphics g, IContainer container, FeedbackRect.Dtype dtype, double x[], double y[],
			double z[], int option) {
		...
		for (int hitIndex = 0; hitIndex < len; hitIndex++) {
			...
			if (showPoint) {
				...
				String s = vecStr("Gemc Hit [" + (hitIndex + 1) + "] " + "  sect: " + sector + "  ", labXYZ[0],
						labXYZ[1], labXYZ[2]) + " cm";
		...
		for (FeedbackRect rr : _fbRects) {
			...
			if (contains && (rr.hitIndex >= 0)) {
				...
				if ((strip > 0) && (stack > 0) && (view > 0)) {
					String s = DataSupport.trueColor + "Gemc Hit " + " plane " + DataDrawSupport.EC_PLANE_NAMES[stack]
							+ " type " + DataDrawSupport.EC_VIEW_NAMES[view] + " strip " + strip;
