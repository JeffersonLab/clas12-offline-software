...
public class FMTGeometry {
	...
	public static double[][] FVT_stripsXloc = new double[FVT_Nstrips][2]; // Give the local end-points x-coordinates of
																			// the strip segment
	public static double[][] FVT_stripsYloc = new double[FVT_Nstrips][2]; // Give the local end-points y-coordinates of
																			// the strip segment
	public static double[] FVT_stripsXlocref = new double[FVT_Nstrips]; // Give the local ref-points x-coordinates of
																		// the strip segment
	public static double[] FVT_stripsYlocref = new double[FVT_Nstrips]; // Give the local ref-points y-coordinates of
																		// the strip segment
	public static double[][][] FVT_stripsX = new double[FVT_Nlayers][FVT_Nstrips][2]; // Give the end-points
																						// x-coordinates of the strip
																						// segment rotated in the
																						// correct frame for the layer
	public static double[][][] FVT_stripsY = new double[FVT_Nlayers][FVT_Nstrips][2]; // Give the end-points
																						// y-coordinates of the strip
																						// segment
		...
		for (int i = 0; i < FVT_Nstrips; i++) {
			...
			case 1:
				...
				if (Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole < 1) {
					...
					FVT_stripsXloc[i][1] = -FVT_Beamhole
							* Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole));
					FVT_stripsXloc[i][0] = -FVT_stripslength[i];
					FVT_stripsXlocref[i] = -FVT_stripslength[i] / 2
							- FVT_Beamhole * Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole));
				}
			...
			case 3:
				...
				if (Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole < 1) {
					FVT_stripslength[i] = FVT_Rmax * Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Rmax))
							- FVT_Beamhole * Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole));
					FVT_stripsXloc[i][0] = FVT_Beamhole
							* Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole));
					FVT_stripsXloc[i][1] = FVT_stripslength[i];
					FVT_stripsXlocref[i] = FVT_stripslength[i] / 2
							+ FVT_Beamhole * Math.sin(Math.acos(Math.abs(FVT_stripsYloc[i][0]) / FVT_Beamhole));
			...
			for (int j = 0; j < FVT_Nlayers; j++) { // X sign flip
				FVT_stripsX[j][i][0] = -(FVT_stripsXloc[i][0] * Math.cos(FVT_Alpha[j])
						+ FVT_stripsYloc[i][0] * Math.sin(FVT_Alpha[j]));
				FVT_stripsY[j][i][0] = -FVT_stripsXloc[i][0] * Math.sin(FVT_Alpha[j])
						+ FVT_stripsYloc[i][0] * Math.cos(FVT_Alpha[j]);
				FVT_stripsX[j][i][1] = -(FVT_stripsXloc[i][1] * Math.cos(FVT_Alpha[j])
						+ FVT_stripsYloc[i][1] * Math.sin(FVT_Alpha[j]));
				FVT_stripsY[j][i][1] = -FVT_stripsXloc[i][1] * Math.sin(FVT_Alpha[j])
						+ FVT_stripsYloc[i][1] * Math.cos(FVT_Alpha[j]);
			}

			// System.out.println(Constants.getLocalRegion(i)+" strip-1 = "+i+" x'
			// "+FVT_stripsXloc[i][1]+" y' "+FVT_stripsYloc[i][1]+" length
			// "+FVT_stripslength[i]+" FVT_Beamhole "+FVT_Beamhole);
		}
	...
	private static int getLocalRegion(int i) {
		// To represent the geometry we divide the barrel micromega disk into 3 regions
		// according to the strip numbering system.
		// Here i = strip_number -1;
		// Region 1 is the region in the negative x part of inner region: the strips
		// range is from 1 to 320 ( 0 <= i < 320)
		// Region 2 is the region in the negative y part of outer region: the strips
		// range is from 321 to 512 ( 320 <= i < 512)
		// Region 3 is the region in the positive x part of inner region: the strips
		// range is from 513 to 832 ( 512 <= i < 832)
		// Region 4 is the region in the positive y part of outer region: the strips
