...
public class GeometryReportView extends BaseView implements ActionListener {
	...
	public GeometryReportView() {
		super(PropertySupport.TITLE, "Geometry Report", PropertySupport.ICONIFIABLE, true, PropertySupport.MAXIMIZABLE,
				true, PropertySupport.CLOSABLE, true, PropertySupport.RESIZABLE, true, PropertySupport.WIDTH, 900,
				PropertySupport.HEIGHT, 900, PropertySupport.VISIBLE, false);
		...
		for (int sect = 1; sect <= 6; sect++) {
			...
			for (int superlay = 1; superlay <= 6; superlay++) {
				...
				for (int lay = 1; lay <= 6; lay++) {
					...
					println(MCOLOR.M_RED,
							"  wire      x1         y1          z1          x2          y2          z2          length");
					...
					for (int wire = 1; wire <= 112; wire++) {
						...
						String s = String.format("  %3d  %10.5f  %10.5f  %10.5f  %10.5f  %10.5f  %10.5f  %10.5f", wire,
								x1, y1, z1, x2, y2, z2, len);
		...
		for (int layer = 1; layer <= 8; layer++) {
			...
			for (int sect = 1; sect <= numsect; sect++) {
				...
				for (int strip = 1; strip <= 256; strip++) {
					...
					double y2 = line3D.end().y();
					double z2 = line3D.end().z();
					String s = String.format("  %3d  %10.5f  %10.5f  %10.5f  %10.5f  %10.5f  %10.5f ", strip, x1, y1,
							z1, x2, y2, z2);
