...
public class ECAL {
	 ...
	 * @param stack the 1-based stack (inner = 1, outer = 2) index
	 * @param view  the 1-based strip type (u, v, w) = (1, 2, 3)
	 * @param strip the 1-based strip
	 * @return the list of matching hits
	 */
	public static Vector<HitRecord> matchingHits(int sect, int stack, int view, int strip) {
		...
		for (int i = 0; i < hitCount; i++) {
			if ((sect == sectors[i]) && (stack == stacks[i]) && (view == views[i]) && (strip == strips[i])) {
				hits.add(new HitRecord(avgX, avgY, avgZ, i, sect, stack, view, strip));
			}
