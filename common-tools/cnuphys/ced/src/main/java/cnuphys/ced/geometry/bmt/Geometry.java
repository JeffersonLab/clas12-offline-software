...
public class Geometry {
	...
	// The inner (i.e. closest to beam in double layer) layer contain longitudinal
	// strips oriented in the Z direction.
	// This is called the Z layer.
	// The outer layer contain arched strips at a non-constant pitch in Z. This is
	// called the C layer.
	...
	public int getDetectorIndex(int sector) {
		...
		int DetIdx = -1;
		/*
		 * if(sector == 1) DetIdx = 2; if(sector == 2) DetIdx = 1; if(sector == 3)
		 * DetIdx = 0;
		 */
		DetIdx = sector - 1;
		return DetIdx;
	...
	public double CRZStrip_GetPhi(int sector, int layer, int strip) {
		...
		// For CRZ, this function returns the angle to localize the center of strip
		// "num_strip" for the "num_detector"
		...
		// double
		// angle=Constants.getCRZEDGE1()[num_region][num_detector]+(Constants.getCRZXPOS()[num_region]+(Constants.getCRZWIDTH()[num_region]/2.+num_strip*(Constants.getCRZWIDTH()[num_region]+Constants.getCRZSPACING()[num_region])))/Constants.getCRZRADIUS()[num_region];
	...
	public int getZStrip(int layer, double angle) { // the angle is the Lorentz uncorrected angle
		...
		double strip_calc = ((angle - Constants.getCRZEDGE1()[num_region][num_detector])
				* Constants.getCRZRADIUS()[num_region] - Constants.getCRZXPOS()[num_region]
				- Constants.getCRZWIDTH()[num_region] / 2.)
				/ (Constants.getCRZWIDTH()[num_region] + Constants.getCRZSPACING()[num_region]);
	...
	private double CRZ_GetZStrip(int layer) {
		...
		double zc = Constants.getCRZZMIN()[num_region] + Constants.getCRZOFFSET()[num_region]
				+ Constants.getCRZLENGTH()[num_region] / 2.;
		return zc; // in mm
	}
	...
	public double CRC_GetBeginStrip(int sector, int layer) {
		...
		// For CRC, this function returns the angle to localize the beginning of the
		// strips
	...
	public double CRCStrip_GetZ(int layer, int strip) {
		...
		double zc = Constants.getCRCZMIN()[num_region] + Constants.getCRCOFFSET()[num_region]
				+ Constants.getCRCWIDTH()[num_region][group] / 2.;
	...
	public int getCStrip(int layer, double trk_z) {
		...
		Z_lowBound[0] = Constants.getCRCWIDTH()[num_region][0] / 2.; // the lower bound is the zMin+theOffset with half
																		// the width
		Z_uppBound[0] = Z_lowBound[0] + (Constants.getCRCGROUP()[num_region][0] - 1)
				* (Constants.getCRCWIDTH()[num_region][0] + Constants.getCRCSPACING()[num_region]);
		NStrips[0] = Constants.getCRCGROUP()[num_region][0];
		for (int i = 1; i < len; i++) {
			Z_lowBound[i] = Z_uppBound[i - 1] + Constants.getCRCWIDTH()[num_region][i - 1] / 2.
					+ Constants.getCRCSPACING()[num_region] + Constants.getCRCWIDTH()[num_region][i] / 2.;
			Z_uppBound[i] = Z_lowBound[i] + (Constants.getCRCGROUP()[num_region][i] - 1)
					* (Constants.getCRCWIDTH()[num_region][i] + Constants.getCRCSPACING()[num_region]);

			NStrips[i] = NStrips[i - 1] + Constants.getCRCGROUP()[num_region][i];

			if (z >= Z_lowBound[i] && z <= Z_uppBound[i]) {
				strip_group = i;
				ClosestStrip = 1 + (int) (Math.round(((z - Z_lowBound[strip_group])
						/ (Constants.getCRCWIDTH()[num_region][strip_group] + Constants.getCRCSPACING()[num_region]))))
						+ NStrips[i - 1];
	...
	public double getSigmaLongit(int layer, double x, double y) { // sigma for C-detector
		...
		int num_region = (layer + 1) / 2 - 1; // region index (0...2) 0=layers 1&2, 1=layers 3&4, 2=layers 5&6
		double sigma = Constants.SigmaMax
				* Math.sqrt((Math.sqrt(x * x + y * y) - Constants.getCRCRADIUS()[num_region] + Constants.hStrip2Det)
						/ Constants.hDrift);
	...
	public double getSigmaAzimuth(int layer, double x, double y) { // sigma for Z-detectors
		...
		int num_region = (layer + 1) / 2 - 1; // region index (0...2) 0=layers 1&2, 1=layers 3&4, 2=layers 5&6double
												// Z0=0;
		double sigma = Constants.SigmaMax
				* Math.sqrt((Math.sqrt(x * x + y * y) - Constants.getCRZRADIUS()[num_region] + Constants.hStrip2Det)
						/ Constants.hDrift / Math.cos(Constants.getThetaL()));
	 ...
	 * @return X[] = the smeared position x = X[0], y = X[1], z = X[2] taking the
	 *         Lorentz angle into account
	 */
	public double[] smearedPosition(int layer, double x, double y, double z) {
		...
		if (layer % 2 == 1) {// Z layer
			...
			double phicorr = (this.randomGaus(0, sigma, rand) / Math.cos(Constants.getThetaL())
					- (Math.sqrt(x * x + y * y) - Constants.getCRZRADIUS()[num_region] + Constants.hStrip2Det)
							* Math.tan(Constants.getThetaL()))
					/ Constants.getCRZRADIUS()[num_region];
	...
	public int isInDetector(int layer, double angle) {
		...
		for (int i = 0; i < 3; i++) {
			...
			angle_i = Constants.getCRCEDGE1()[num_region][i]
					+ Constants.getCRCXPOS()[num_region] / Constants.getCRCRADIUS()[num_region];
			angle_f = Constants.getCRCEDGE1()[num_region][i]
					+ (Constants.getCRCXPOS()[num_region] + Constants.getCRCLENGTH()[num_region])
							/ Constants.getCRCRADIUS()[num_region];
...
//		coords[5] = (float)zMax;
	...
	public static void main(String arg[]) {
		...
		/*
		 * int num_region = (int) (layer+1)/2 - 1; // region index (0...2) 0=layers 1&2,
		 * 1=layers 3&4, 2=layers 5&6 int strip_group = 0; int ClosestStrip =-1;
		 ...
		 * Constants.CRCSPACING[num_region]); NStrips[0] =
		 * Constants.CRCGROUP[num_region][0]; for(int i =1; i< len; i++) { Z_lowBound[i]
		 * = Z_uppBound[i-1] + Constants.CRCWIDTH[num_region][i-1]/2. +
		 * Constants.CRCSPACING[num_region] + Constants.CRCWIDTH[num_region][i]/2.;
		 ...
		 * [0]+", "+X[1]+" , "+X[2]); System.out.println(geo.getZStrip(5,
		 * Math.atan2(Constants.CRZRADIUS[2],0 ))); System.out.println(geo.getZStrip(5,
		 * Math.atan2(X[1],X[0]))); System.out.println(Math.toDegrees(
		 * geo.CRZStrip_GetPhi(1,6, geo.getZStrip(5, Math.atan2(X[1],X[0]))) )); int
		 * theMeasuredZStrip = geo.getZStrip(5, Math.atan2(X[1],X[0])); // start reco
		 * double theMeasuredPhi = geo.CRZStrip_GetPhi(1,6,theMeasuredZStrip); double
		 * theLorentzCorrectedAngle = geo.LorentzAngleCorr( theMeasuredPhi, 6);
		 * System.out.println(" corrected phi = "+Math.toDegrees(
		 * theLorentzCorrectedAngle)); int theLorentzCorrectedStrip = geo.getZStrip(5,
		 * theLorentzCorrectedAngle);
		 * System.out.println(theMeasuredZStrip+" "+theLorentzCorrectedStrip);
		 */
		/*
		 * double phiC = geo.CRZStrip_GetPhi(3,6,216); double x =
		 * Constants.CRCRADIUS[2]*Math.cos(phiC); double y =
		 * Constants.CRCRADIUS[2]*Math.sin(phiC); int theMeasuredCStrip =
		 * geo.getCStrip(6,X[2]); double z = geo.CRCStrip_GetZ(6,309);
		 * System.out.println(x+", "+y+", "+z);
		 */
		// List<double[]> Hits = geo.GEMCBMTHits(layer, sector, -199.89230321711165 ,
		// 93.78543124898611 , -164.52000000000007, .1);
