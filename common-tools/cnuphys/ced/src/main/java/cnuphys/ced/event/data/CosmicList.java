...
public class CosmicList extends Vector<Cosmic> {
	...
	public CosmicList(String bankName) {
		...
		length = checkArrays(id, chi2, phi, theta, trkline_yx_interc, trkline_yx_slope, trkline_yz_interc,
				trkline_yz_slope);
		if (length < 0) {
...
//				float trkline_yz_interc, float trkline_yz_slope) {
	...
	private int checkArrays(short[] id, float[] chi2, float[] phi, float[] theta, float[] trkline_yx_interc,
			float[] trkline_yx_slope, float[] trkline_yz_interc, float[] trkline_yz_slope) {

		if ((id == null) || (chi2 == null) || (phi == null) || (theta == null) || (trkline_yx_interc == null)
				|| (trkline_yx_slope == null) || (trkline_yz_interc == null) || (trkline_yz_slope == null)) {

			_error = "Unexpected null array when creating CosmicList: " + "id = null: " + (id == null)
					+ " chi2 == null: " + (chi2 == null) + " theta == null: " + (theta == null) + " phi == null: "
					+ (phi == null) + " trkline_yx_interc == null: " + (trkline_yx_interc == null)
					+ " trkline_yx_slope == null: " + (trkline_yx_slope == null) + " trkline_yz_interc == null: "
					+ (trkline_yz_interc == null) + " trkline_yz_slope == null: " + (trkline_yz_slope == null);
	...
	private boolean lengthMismatch(short[] id, float[] array, String name) {
		...
		if (id.length != array.length) {
			_error = "ID length: " + id.length + " does not match " + name + " length: " + array.length
					+ " when creating CosmicList";
			return true;
		}
		return false;
	}
