	...
	private int checkArrays(byte[] sector, byte[] superlayer, float[] x1, float[] z1, float[] x2, float[] z2) {
		if ((sector == null) || (superlayer == null) || (x1 == null) || (z1 == null) || (x2 == null) || (z2 == null)) {
			_error = "Unexpected null array when creating SegmentList: " + "sector = null: " + (sector == null)
					+ " superlayer == null: " + (superlayer == null) + " x1 == null: " + (x1 == null) + " z1 == null: "
					+ (z1 == null) + " x2 == null: " + (x2 == null) + " z2 == null: " + (z2 == null);
			return -1;
		}
		...
		if (superlayer.length != sector.length) {
			_error = "Sector length: " + sector.length + " does not match super length: " + superlayer.length
					+ " when creating SegmentList";
			return -1;
		}

		if (x1.length != sector.length) {
			_error = "Sector length: " + sector.length + " does not match x1 length: " + x1.length
					+ " when creating SegmentList";
			return -1;
		}

		if (z1.length != sector.length) {
			_error = "Sector length: " + sector.length + " does not match z1 length: " + z1.length
					+ " when creating SegmentList";
			return -1;
		}

		if (x2.length != sector.length) {
			_error = "Sector length: " + sector.length + " does not match x2 length: " + x2.length
					+ " when creating SegmentList";
			return -1;
		}

		if (z2.length != sector.length) {
			_error = "Sector length: " + sector.length + " does not match z2 length: " + z2.length
					+ " when creating SegmentList";
			return -1;
		}
