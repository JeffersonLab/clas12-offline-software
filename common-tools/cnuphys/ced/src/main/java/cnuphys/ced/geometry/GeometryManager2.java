...
public class GeometryManager2 {
	...
	public static synchronized void load(int runNb, String var) {
		// the geometry is different is hardware and geometry... until GEMC gets updated
	...
	private static Point3D rotateFromTSCtoLabC(double x, double y, double z, int sector) {
		...
		double rx = rxs * Math.cos((sector - 1) * Math.toRadians(60.))
				- y * Math.sin((sector - 1) * Math.toRadians(60.));
		double ry = rxs * Math.sin((sector - 1) * Math.toRadians(60.))
				+ y * Math.cos((sector - 1) * Math.toRadians(60.));
