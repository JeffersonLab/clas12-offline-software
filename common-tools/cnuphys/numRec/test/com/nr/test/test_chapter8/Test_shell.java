...
public class Test_shell {
  ...
  public void test() {
    ...
    Sorter.shell(y,N/2);           // sort first half of array
    for (i=0;i<N/2-1;i++) localflag = localflag || (y[i] > y[i+1]);
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** shell: First half of array is not correctly ordered");
    ...
    globalflag = globalflag || localflag;
    if (localflag) {
      fail("*** shell: Second half of array is not correctly ordered");
