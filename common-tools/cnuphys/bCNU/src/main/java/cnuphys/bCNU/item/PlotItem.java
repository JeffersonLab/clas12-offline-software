...
//			    String yLabel) {
	 ...
	 */
	public PlotItem(LogicalLayer layer, Point2D.Double location, int width, int height) {
	 ...
	 */
	public PlotItem(LogicalLayer layer, Point2D.Double location, int width, int height, String title, String xLabel,
			String yLabel, DataSet dataSet) {
...
//		getContainer().getComponent().addMouseListener(_plotCanvas);
	 ...
	 */
	public static PlotItem createHistogram(LogicalLayer layer, Point2D.Double location, int width, int height,
			String title, String xLabel, String yLabel, String histoCurveName, double minValue, double maxValue,
			int numBin) {

		HistoData h1 = new HistoData(histoCurveName, minValue, maxValue, numBin);
		...
		return new PlotItem(layer, location, width, height, title, xLabel, yLabel, data);
	}
