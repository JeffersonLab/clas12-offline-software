...
public class TitledPanel extends JPanel {
	...
	private TitledPanel(String aTitle, Component aContent, int aBorderFlag, Insets aInsets) {
		...
		if ((aBorderFlag & SOUTH) != 0) {
			north_content_south_panel.add(new JSeparator(SwingConstants.HORIZONTAL), BorderLayout.SOUTH);
		}
		if (aInsets != null) {
			north_content_south_panel.setBorder(BorderFactory.createEmptyBorder(0, aInsets.left, 0, aInsets.right));
		}
		...
		if (aInsets != null) {
			setBorder(BorderFactory.createEmptyBorder(aInsets.top, 0, aInsets.bottom, 0));
		}
	}
