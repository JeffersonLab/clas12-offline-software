...
public enum FitType {
    ...
    private static final String DARKRED = X11Colors
	    .getX11ColorAsHex("Dark RED");
    private static final String DARKGREEN = X11Colors
	    .getX11ColorAsHex("Dark GREEN");

    /**
     * A map for the names of the fit types
     */
    public static EnumMap<FitType, String> names = new EnumMap<FitType, String>(
	    FitType.class);
     ...
     * 
     * @param object
     *            the fit object that will have to be cast appropriately
	...
	case POLYNOMIAL:
	    sb.append(header("Polynomial Fit")
		    + descript("y = " + _SUM + sub("A", "n")
			    + "&thinsp;x<SUP>n</SUP>"));
		...
		for (int i = 0; i < fsvd.a.length; i++) {
		    sb.append(pmString("A<SUB>" + i + "</SUB>", fsvd.a[i],
			    Math.sqrt(fsvd.covar[i][i]))
			    + "<BR>");
		}
		sb.append(errorType(curveFit));
	...
	case ERF:
	    sb.append(header("Erf Fit:")
		    + descript("y = A + B&thinsp;Erf[(x-" + _MU + ")/S" + "]"));
		...
		sb.append(colorStr("S/" + UnicodeSupport.SQRT + "2 = "
			+ valStr(a[3] / Math.sqrt(2)), "gray")
			+ _EOL);
		sb.append(errorType(curveFit));
	...
	case ERFC:
	    sb.append(header("Erfc Fit:")
		    + descript("y = A + B&thinsp;Erfc[(x-" + _MU + ")/S" + "]"));
	...
	case ALTPOLYNOMIAL:
	    sb.append(header("Alt Polynomial Fit:")
		    + descript("y = " + _SUM + sub("A", "n")
			    + "&thinsp;x<SUP>n</SUP>"));
	    sb.append(info("Polynomial Order: " + curveFit.getPolynomialOrder()));
		...
		for (int i = 0; i < faltp.a.length; i++) {
		    sb.append(pmString(sub("A", i), faltp.a[i],
			    Math.sqrt(faltp.covar[i][i]))
			    + "<BR>");
		}
		sb.append(errorType(curveFit));
	...
	case GAUSSIANS:
		...
		for (int i = 0; i < fg.a.length / 3; i++) {
		    int j = 3 * i;
		    sb.append(pmString(sub("A", i), fg.a[j],
			    Math.sqrt(fg.covar[j][j]))
			    + _EOL);
		    j++;
		    sb.append(pmString(sub(_MU, i), fg.a[j],
			    Math.sqrt(fg.covar[j][j]))
			    + _EOL);
		    j++;
		    sb.append(pmString(sub("S", i), fg.a[j],
			    Math.sqrt(fg.covar[j][j]))
			    + _EOL);
		}
		sb.append(errorType(curveFit));
	...
	case POLYPLUSGAUSS:
	    sb.append(header("Gaussian & Polynomial Fit:")
		    + descript("y = " + _SUM + sub("A", "n")
			    + "&thinsp;exp{-[(x-" + sub(_MU, "n") + ")/"
			    + sub("S", "n") + "]<SUP>2</SUP>} + " + _SUM
			    + sub("D", "m") + "&thinsp;x<SUP>m</SUP>"));
	    sb.append(info("Number of Gaussians: " + curveFit.getNumGaussian()));
	    sb.append(info("Polynomial Order: " + curveFit.getPolynomialOrder()));
		...
		for (int i = 0; i < fpg._numGauss; i++) {
		    int j = 3 * i;
		    sb.append(pmString(sub("A", i), fpg.a[j],
			    Math.sqrt(fpg.covar[j][j]))
			    + "<BR>");
		    j++;
		    sb.append(pmString(sub(_MU, i), fpg.a[j],
			    Math.sqrt(fpg.covar[j][j]))
			    + "<BR>");
		    j++;
		    sb.append(pmString(sub("S", i), fpg.a[j],
			    Math.sqrt(fpg.covar[j][j]))
			    + "<BR>");
		}
		// now the poly
		sb.append(colorStr("<b>Polynomial Parameters</b>", "blue")
			+ _EOL);
		for (int i = 3 * fpg._numGauss; i < fpg.a.length; i++) {
		    int j = i - 3 * fpg._numGauss;
		    sb.append(pmString(sub("D", j), fpg.a[i],
			    Math.sqrt(fpg.covar[i][i]))
			    + _EOL);
	...
	String htmlstr = "<font style=\"COLOR: " + fg + "; "
		+ "BACKGROUND-COLOR: " + bg + "\">" + s + "</font>";
	return htmlstr;
    }
	...
	return vname + " = " + vstr
		+ colorStr("&thinsp;&plusmn;&thinsp;" + vsigstr, "black");
    }
