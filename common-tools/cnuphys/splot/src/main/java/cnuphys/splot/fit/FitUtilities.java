...
public class FitUtilities {
     ...
     *            the fit data
     */
    public static void fitGaussPlusPoly(Fit fit)
     ...
     *            the index array
     * @param vals
     *            the array that will be rearranged
     */
     ...
     * @param v
     *            the input array
     * @return a double[4] containing, in order, the mean, min, max, and
     *         standard deviation
     */
     ...
     * creates a new array and assigns a value of 1 to each error. It does the
     * same if the sigma array has all zero values. The real troubling case is
     * when sigma has some zero values. This will cause many fits to fail. So in
     * that case it repleaces all zero values with the mean value. At that point
     * the fit should be considered suspect.
     * 
     * @param sig
     *            the sigma array, which might be null
     * @param n
	...
	if (hasXErr) {
	    ...
	    parallelSort(fit.getX(), fit.getY(), fit.getSigmaX(),
		    fit.getSigmaY());
	} else {
