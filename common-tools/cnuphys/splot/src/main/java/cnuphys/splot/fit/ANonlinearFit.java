...
public abstract class ANonlinearFit implements IValueGetter {
	...
	_func = getFunc();
	_fitmrq = new Fitmrq(fit.getX(), fit.getY(), fit.getSigmaY(),
		initialGuess(fit), _func, fit.getTolerance());
    ...
    /**
     * This method is an "after the fact" evaluator. After the fit is complete
     * is should be able to use the resulting fit parameters to simply evaluate
     * the function at a given x. The default implementation uses the fitting
     * function _func. This is good in that it guarantees consistency, but the
     * fitting function also computes the derivatives so there is an
     * inefficiency. NOTE: this function is NOT used by the fitting process. It
     * is only for convenience, afterward, such as in plotting.
     */
    @Override
