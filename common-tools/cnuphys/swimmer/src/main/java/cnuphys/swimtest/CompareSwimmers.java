...
public class CompareSwimmers {
	...
	public static void specialCaseTest() {
		...
		try {
			int sect = 2;
			SwimTrajectory traj = swimmer1.sectorSwim(sect, -1, xo, yo, zo, p, theta, phi, zTarg, accuracy, 8, 9,
			...
			double sf[] = tiltedToSector(traj.lastElement()[SwimTrajectory.X_IDX],
					traj.lastElement()[SwimTrajectory.Z_IDX]);
			...
			System.out.println("START IN SECTOR CS: (" + 100 * sc[0] + ", " + 100 * yo + ", " + 100 * sc[1]
					+ ")  theta = " + sTheta + "  phi = " + sPhi);
			System.out.println("  END IN SECTOR CS: (" + 100 * sf[0] + ", "
					+ 100 * traj.lastElement()[SwimTrajectory.Y_IDX] + ", " + 100 * sf[1] + ")");
			System.out.println();
	...
	public static void swimmerVswimmer2Test(long seed, int num) {
		...
		for (int i = 0; i < num / 2; i++) {
			try {
				traj1[i] = swimmer1.swim(charge[i], x0[i], y0[i], z0[i], pTot[i], theta[i], phi[i], zTarg, accuracy, 10,
						10, stepSize, Swimmer.CLAS_Tolerance, hdata);
				traj2[i] = swimmer2.swim(charge[i], x0[i], y0[i], z0[i], pTot[i], theta[i], phi[i], zTarg, accuracy, 10,
						stepSize, Swimmer.CLAS_Tolerance, hdata);
			} catch (RungeKuttaException e) {
				e.printStackTrace();
			}
...
//				double distanceBetweenSaves) {
		...
		time = System.currentTimeMillis() - time;
		System.out.println("Swimmer 1 time: " + timeString(time, num) + "  max step size = " + hdata[2] + "  numStep: "
				+ (traj1[num - 1].size()));
		...
		lastTraj.computeBDL(swimmer1.getProbe());
		System.out.println(
				"**** BDL for swimmer 1 = " + 100 * lastTraj.lastElement()[SwimTrajectory.BXDL_IDX] + "  kG cm");
		System.out.println(
				"**** PATHLENGTH for swimmer 1 = " + 100 * lastTraj.lastElement()[SwimTrajectory.PATHLEN_IDX] + "  cm");
		...
		time = System.currentTimeMillis() - time;
		System.out.println("Swimmer 2 time: " + timeString(time, num) + "  max step size = " + hdata[2] + "  numStep: "
				+ (traj2[num - 1].size()));
		...
		System.out.println(
				"**** BDL for swimmer 2 = " + 100 * lastTraj.lastElement()[SwimTrajectory.BXDL_IDX] + "  kG cm");
		System.out.println(
				"**** PATHLENGTH for swimmer 2 = " + 100 * lastTraj.lastElement()[SwimTrajectory.PATHLEN_IDX] + "  cm");
		...
		for (int i = 0; i < num; i++) {
			try {
				numStep = swimmer2.swim(charge[i], x0[i], y0[i], z0[i], pTot[i], theta[i], phi[i], zTarg, accuracy, 10,
						stepSize, RungeKutta.DEFMAXSTEPSIZE, Swimmer.CLAS_Tolerance, hdata, finalV);
			} catch (RungeKuttaException e) {
				e.printStackTrace();
			}
		}
		time = System.currentTimeMillis() - time;
		System.out.println("Swimmer 2 no traj time " + timeString(time, num) + "  max step size = " + hdata[2]
				+ "  numStep: " + numStep);
		...
		System.out.println("SwimZ time: " + timeString(time, num) + "  max step size = " + hdata[2] / 100.
				+ "  numStep: " + (lastSZR.size()));
...
//		SwimTest.printSwimZ(szTraj[num-1].last(), "Last for swimZ");
		...
		time = System.currentTimeMillis() - time;
		System.out.println("SwimZ no traj time: " + timeString(time, num) + "  max step size = " + hdata[2] / 100.
				+ "  numStep: " + (szTraj[num - 1].size()));
...
//		SwimTest.printSwimZ(stopSV, "Last for swimZ no traj");
		...
		try {
			...
			SwimTest.printSummary("End of Traj for Swimmer 1 Backwards", lastTraj.size(), pB, lastTraj.lastElement(),
					hdata);
			lastTraj.computeBDL(swimmer2.getProbe());
			System.out.println("**** BDL for swimmer 1 Backwards = "
					+ 100 * lastTraj.lastElement()[SwimTrajectory.BXDL_IDX] + "  kG cm");
			System.out.println("**** PATHLENGTH for swimmer 1 backwards = "
					+ 100 * lastTraj.lastElement()[SwimTrajectory.PATHLEN_IDX] + "  cm");
