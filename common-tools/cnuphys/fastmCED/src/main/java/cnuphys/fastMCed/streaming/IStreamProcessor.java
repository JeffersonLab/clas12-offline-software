...
public interface IStreamProcessor extends EventListener {
	...
	/**
	 * A new event in the stream. This occures when FastMCed is not looking event by
	 ...
	 * @param particleHits a list (each entry corresponding to a particle and its
	 *                     trajectory) of particle hits which can be queried for
	 *                     hits in a given detector.
	 * @return StreamingReason.CONTINUE (success) or StreamingReason.FLAG (problem).
	 *         Any consumer returning StreamingReason.FLAG will halt the process and
	 *         cause the GUI to display the event that caused the
	...
	/**
	 * If the processor flags an event, it will be asked to return the reason.
	 * 
