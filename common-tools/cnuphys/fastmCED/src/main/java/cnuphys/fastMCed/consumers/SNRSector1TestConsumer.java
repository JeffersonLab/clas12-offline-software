...
public class SNRSector1TestConsumer extends ASNRConsumer {
	...
	public void update(SNRDictionary dictionary, Dictionary3DPlot plot, List<ParticleHits> particleHits,
			int direction) {
	...
	private void lookForTrack(SNRDictionary dictionary, Dictionary3DPlot plot, int direction) {
		...
		if (snr.segmentsInAllSuperlayers(0, direction)) {
			...
			System.err.println(
					String.format("Error dP = %-6.3f MeV dP/P = %-6.3f%% dTheta = %-6.3f deg  dPhi = %-6.3f deg", dP,
