...
public class SVTConstants {
    ...
    public static void load( ConstantProvider cp ) {
        if( !bLoadedConstants ) {            
            ...
            MODULELEN      = NSENSORS    *    (ACTIVESENLEN + 2*DEADZNLEN)
                            + (NSENSORS    -    1)    *    MICROGAPLEN;
            ...
            PASSIVETHK     = MATERIALDIMENSIONS.get("carbonFiber")[1]
                            + MATERIALDIMENSIONS.get("busCable")[1]
                            + MATERIALDIMENSIONS.get("epoxyAndRailAndPads")[1];
            SECTORLEN      = MATERIALDIMENSIONS.get("heatSink")[2]
                           + MATERIALDIMENSIONS.get("rohacell")[2];
            // construct from material thicknesses instead
            double layerGapThk = MATERIALDIMENSIONS.get("rohacell")[1]
                                                    + 2*PASSIVETHK;
            
            if( VERBOSE ) System.out.printf( "LAYERGAPTHK (CCDB)      = % 8.3f\n", LAYERGAPTHK );
            if( VERBOSE ) System.out.printf( "layerGapThk (MATERIALS) = % 8.3f\n", layerGapThk );
            LAYERGAPTHK = layerGapThk;
            if( VERBOSE ) System.out.println( "set LAYERGAPTHK to layerGapThk");
    ...
    public static void loadAlignmentShifts( ConstantProvider cp ) {
        System.out.println("reading alignment shifts from database BEEP BOOP BWAARRRP different");
    ...
    */
    ...
    /**
     * Converts Region, Sector and Fiducial indices to linear index.
    ...
    public static int[] convertIndex2RegionSectorFiducial( int aSurveyIndex ) throws IllegalArgumentException {
        ...
        for( int i = 0; i < NREGIONS; i++ ) {
            ...
            if( i == NREGIONS-1 ) {
                l1 = l0 + NSECTORS[NREGIONS-1] * NFIDUCIALS; }
            if( l0 <= aSurveyIndex && aSurveyIndex <= l1-1 ) {
                region = i; break; }
        }        
    ...
    /**
     * Converts Region and Sector indices to linear index.
     * 
     * @param aRegion an index starting from 0
     * @param aSector an index starting from 0
    ...
    public static double[][] getDataAlignmentSectorShift() {
        if( SECTORSHIFTDATA == null ) {
            System.err.println( "error: SVTConstants.getDataAlignmentSectorShift: SECTORSHIFTDATA requested is null" ); }
            // System.exit(-1);
