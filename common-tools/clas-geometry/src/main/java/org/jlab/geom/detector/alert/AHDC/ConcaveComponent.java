...
public abstract class ConcaveComponent extends AbstractComponent {
    ...
    protected ConcaveComponent(int componentId, List<Point3D> firstFace, List<Point3D> secondFace) {
        ...
        System.out.println("We are inside ConcaveComponent constructor!");
       ...
       npoints = firstFace.size();
        ...
        if (!isCoplanarConvex(firstFace))
            System.out.println("Convexity check inside ConcaveComponent constructor. First face is not convex and coplanar: componentId="+componentId);
        if (!isCoplanarConvex(secondFace))
            System.out.println("Convexity check inside ConcaveComponent constructor. Second face is not convex and coplanar: componentId="+componentId);
