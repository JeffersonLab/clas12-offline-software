    ...
    * * * * * * * */
    ...
    public Point3D getPlaneModuleOrigin(int sector, int layer) {
        ...
        Line3d labFrameLine = localLine.transformed(SVTConstants.getLabFrame( (layer-1)/2,
                                                                              sector-1,
                                                                              SVTConstants.LAYERRADIUS[ (layer-1)/2 ][ (layer-1)%2],
                                                                              SVTConstants.Z0ACTIVE[    (layer-1)/2 ] ));
 	...
 	AlignmentFactory.applyShift(labFrameLine.origin(),SVTConstants.getDataAlignmentSectorShift()[SVTConstants.convertRegionSector2Index( (layer-1)/2,sector-1)], SVTAlignmentFactory.getIdealFiducialCenter( (layer-1)/2, sector-1),scaleT,scaleR );
        AlignmentFactory.applyShift(labFrameLine.end(),   SVTConstants.getDataAlignmentSectorShift()[SVTConstants.convertRegionSector2Index( (layer-1)/2,sector-1)], SVTAlignmentFactory.getIdealFiducialCenter( (layer-1)/2, sector-1),scaleT,scaleR );
    ...
    public Point3D transformToFrame(int sector, int layer, double x, double y, double z, String frame, String MiddlePlane) {
        ...
        if (MiddlePlane.equals("middle")) {
            if ((layer - 1) % 2 == 0) { // for a cross take the bottom layer;
                gap = SVTConstants.LAYERRADIUS[layer/2][layer%2]
                    - SVTConstants.LAYERRADIUS[(layer-1)/2][(layer-1)%2]; //gap+=0.004; makes geom align with mc
            }
        }
        Point3D transf = null;
        if (frame.equals("lab")) {
        ...
        if (frame.equals("local")) {
 	    ...
 	    double scaleT = 1.0, scaleR = 1.0;// scale factors used for visualization. Not relevant here.
 	    AlignmentFactory.applyInverseShift( glLine.origin(), SVTConstants.getDataAlignmentSectorShift()[SVTConstants.convertRegionSector2Index( (layer-1)/2,sector-1 )], SVTAlignmentFactory.getIdealFiducialCenter( (layer-1)/2, sector-1 ), scaleT, scaleR );
 	    AlignmentFactory.applyInverseShift( glLine.end(), SVTConstants.getDataAlignmentSectorShift()[SVTConstants.convertRegionSector2Index( (layer-1)/2,sector-1 )], SVTAlignmentFactory.getIdealFiducialCenter( (layer-1)/2, sector-1 ), scaleT, scaleR );
  
            Line3d localLine = glLine.transformed(SVTConstants.getLabFrame( (int)((layer+1)/2) -1,
                                                                            sector-1,
                                                                            SVTConstants.LAYERRADIUS[(layer-1)/2][(layer-1)%2]+gap/2, 
                                                                            SVTConstants.Z0ACTIVE[(layer-1)/2] ).invert());
    ...
    public double[] getCrossPars(int sector, int upperlayer, double s1, double s2, String frame, Vector3D trkDir) {
        ...
        double Glob_rangl = ((double) -(sector - 1) / (double) SVTConstants.NSECTORS[layerIdx/2])
    ...
    public Point3D intersectionOfHelixWithPlane(int layer, int sector, Helix helix) {
        ...
        double RMin = SVTConstants.LAYERRADIUS[(layer-1)/2][(layer-1)%2];
    ...
    public Point3D recalcCrossFromTrajectoryIntersWithModulePlanes(int s, double s1, double s2, int l1, int l2,
        ...
        double l = (SVTConstants.LAYERRADIUS[(l1-1)/2][(l1-1)%2] - SVTConstants.LAYERRADIUS[(l2-1)/2][(l2-1)%2]) / (n.dot(t));
