...
public class HelixCrossListFinder {
    ...
    private void MatchToRegion1(Seed s, ArrayList<Cross> R1Crosses, org.jlab.rec.cvt.svt.Geometry svt_geo, Swim swimmer) {
        ...
        double gap = SVTConstants.LAYERRADIUS[0][1]
                    - SVTConstants.LAYERRADIUS[0][0]; //gap+=0.004; makes geom align with mc
        Point3D trkAtR1 = cand.get_helix().getPointAtRadius(SVTConstants.LAYERRADIUS[0][0] + gap / 2);
        ...
        Point3D trkAtL1 = cand.get_helix().getPointAtRadius(SVTConstants.LAYERRADIUS[0][0]);
        Point3D trkAtL2 = cand.get_helix().getPointAtRadius(SVTConstants.LAYERRADIUS[0][0] + gap);
    ...
    private double calcCentErr(Cross c, Cluster Cluster1, Geometry svt_geo) {
        ...
        return Cluster1.get_ResolutionAlongZ(Z, svt_geo) / (SVTConstants.READOUTPITCH / Math.sqrt(12.));
    }
