...
public class GeometryOld {
 ...
 /*
        ...
        DatabaseConstantProvider cp = new DatabaseConstantProvider(190,"default");
        ...
        // load alignment shifts and print them out;
        //SVTConstants.loadAlignmentShifts( cp );

        // set parameters.
        int myStrip  = 1; // pick a strip, layer, sector
        int myLayer  = 1;
        int mySector = 1;
        //int myModule = 1; // corresponding module and region.
        //int myRegion = 2;

        System.out.format("\nmyStrip = %1d, mySector = %1d, myLayer = %1d\n\n",myStrip,mySector,myLayer);

        // set indices. the method wants the index not the acutal number which differ by 1.
        int myStripIndex  = myStrip-1;
        int myLayerIndex  = myLayer-1;
        int mySectorIndex = mySector-1;

        // shifted strip in lab coordinates
        Line3d myCLASstriplineShifted  = svtIdealStripFactory.getIdealStrip(myLayerIndex, mySectorIndex, myStripIndex);
        System.out.println( "myLabIdeal   = "+myCLASstriplineShifted.toString());

        // ideal strip in lab coordinates
        Line3d myCLASstriplineIdeal  = svtIdealStripFactory.getIdealStrip(myLayerIndex+1, mySectorIndex, myStripIndex);
        System.out.println( "myLabIdeal     = "+myCLASstriplineIdeal.toString());
    }
