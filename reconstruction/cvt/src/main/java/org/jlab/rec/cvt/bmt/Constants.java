 ...
 * This version is for the last region of the BMT only.
   ...
   The angles are defined with theZ-axis oriented from the accelerator to the beam dump.
    ...
    public static synchronized void setThetaL(int layer, int sector) {
        if (org.jlab.rec.cvt.Constants.isCosmicsData() == true) ThetaL = 0;
    	else ThetaL = Math.toRadians(org.jlab.rec.cvt.bmt.Lorentz.GetLorentzAngle(E_DRIFT[layer-1][sector-1],Math.abs(org.jlab.rec.cvt.Constants.getSolenoidscale()*50)));
