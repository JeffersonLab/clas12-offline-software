    ...
    public Trajectory findTrajectory(Track trk, Swim swimmer, String isFinal) {
        ...
        for (int a = 0; a < Sectors.length; a++) {
            if (Sectors[a] == 0) {
                ...
                int sec = Constants.getInstance().SVTGEOMETRY.getSector(a+1, I);
...
//            this.fill_HelicalTrkAngleWRTSVTPlane(sector, layer, dir, svt_geo, stVec);
            Vector3D localDir = Constants.getInstance().SVTGEOMETRY.getLocalTrack(layer, sector, dir);
            stVec.setTrkPhiAtSurface(localDir.phi());
            stVec.setTrkThetaAtSurface(localDir.theta());
            stVec.setTrkToModuleAngle(Constants.getInstance().SVTGEOMETRY.getLocalAngle(layer, sector, dir));
            stVec.setCalcCentroidStrip(Constants.getInstance().SVTGEOMETRY.calcNearestStrip(inters[0]*10, inters[1]*10, inters[2]*10, layer, sector));
        ...
        path = 0;
        //BMT
        for (int l = SVTGeometry.NLAYERS; l < SVTGeometry.NLAYERS + Constants.getInstance().BMTGEOMETRY.getNLayers(); l++) {
            ...
            if(inters!=null || l==SVTGeometry.NLAYERS) { // don't swim if previous layers was not reached
                ...
                double radius  = Constants.getInstance().BMTGEOMETRY.getRadiusMidDrift(layer)/10;                
                ...
                int sector = Constants.getInstance().BMTGEOMETRY.getSector(0, Math.atan2(inters[1],inters[0]));
                
                Line3D axis    = Constants.getInstance().BMTGEOMETRY.getAxis(layer, sector);
...
//                    int sector = bmt_geo.getSector(layer,phiPos);
                ...
                Vector3D localDir = Constants.getInstance().BMTGEOMETRY.getLocalTrack(layer, sector, pos, dir);
                stVec.setTrkPhiAtSurface(localDir.phi());
                stVec.setTrkThetaAtSurface(localDir.theta());
                stVec.setTrkToModuleAngle(Constants.getInstance().BMTGEOMETRY.getLocalAngle(layer, sector, pos, dir));
                stVec.setCalcCentroidStrip(Constants.getInstance().BMTGEOMETRY.getCstrip(region, 
        ...
        // CTOF
        if(Constants.getInstance().CTOFGEOMETRY!=null && inters!=null) {    //  don't swim to CTOF if swimming to BMT failed
            ...
            inters = swimmer.SwimGenCylinder(new Point3D(0,0,0), new Point3D(0,0,1), radius, Constants.getInstance().SWIMACCURACYCD/10);
            if(inters!=null) {
                // update parameters
                double r = Math.sqrt(inters[0]*inters[0]+inters[1]*inters[1]);
        ...
        // CND
        if(Constants.getInstance().CNDGEOMETRY!=null && inters!=null) {     //  don't swim to CND if swimming to CTOF failed
            for(int ilayer=0; ilayer<Constants.getInstance().CNDGEOMETRY.getSector(0).getSuperlayer(0).getNumLayers(); ilayer++) {
                ...
                swimmer.SetSwimParameters(inters[0], inters[1], inters[2], 
                        ...
                        maxPathLength) ;
                // swim to CTOF
                Point3D center = Constants.getInstance().CNDGEOMETRY.getSector(0).getSuperlayer(0).getLayer(ilayer).getComponent(0).getMidpoint();
