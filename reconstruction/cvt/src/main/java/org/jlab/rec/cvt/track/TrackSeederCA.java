...
//             if the cell has been already used for one track candidate, then skip it
                ...
                if (cell.get_plane().equalsIgnoreCase("XY")) {
                    if (cell.get_c1().is_usedInXYcand() || cell.get_c2().is_usedInXYcand()) {
                        continue;
                    }
                }

                if (cell.get_plane().equalsIgnoreCase("ZR")) {
                    if (cell.get_c1().is_usedInZRcand() || cell.get_c2().is_usedInZRcand()) {
                        continue;
                    }
                }
...
//      	      		  }      				  
                        if (cn.get_state() != neighbour.get_state() - 1) {
                            continue;
                        }
                        if (cn.get_state() >= ms) {
                            ms = cn.get_state();
...
//      	  }
    ...
    public List<Seed> findSeed(List<Cross> svt_crosses, List<Cross> bmt_crosses,
        ...
        for (int i = 0; i < 3; i++) {
            bmtC_crosses.add(new ArrayList<Cross>());
        }
...
//        }
//        System.out.println();
            if (bmtC_crosses.get(sector) == null || bmtC_crosses.get(sector).size() == 0) {
                continue;
            }
...
//        System.out.println("sector" + sector + " len " + zrtracks.size());  
            ...
            for (List<Cross> zrcross : zrtracks) {
                ...
                for (Cross c : svtcrs) {
                    ...
                    double r1 = SVTConstants.LAYERRADIUS[(l1 - 1)/2][(l1 - 1)%2];
                    ...
                    double r2 = SVTConstants.LAYERRADIUS[(l2 - 1)/2][(l2 - 1)%2];
...
//          }
    ...
    public Track fitSeed(List<Cross> VTCrosses,
        ...
        for (int i = 0; i < fitIter; i++) {
            ...
            //if (BMTCrossesZ != null) {//cannot be null -- dereference warning
