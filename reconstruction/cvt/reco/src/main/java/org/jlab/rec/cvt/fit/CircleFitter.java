 ...
 * The algorithm returns Gaussian parameters rho (circle curvature), d (doca to orig.) and phi (at doca).
 * Allows for non-iterative solution.
 * Returns covariance matrix of fitted parameter.
 * 
 * Algorithm Reference:
 * Nuclear Instruments and Methods in Physics Research A305 (1991) 187-191 
 * Effective circle fitting for particle trajectories by V. Karimaki
 *
 */
public class CircleFitter {
	...
	/**
	 * Constructor
	 * Sets the reference point to (0,0)
	 */
	public CircleFitter()
	{
		_xref = 0.;
	 ...
	 * The number of points that are fitted is given by NP.
	...
	public boolean fitStatus(double [] xm, double [] ym, double [] wm, int NP)
	{
		...
		if(Sw<=0.) return false;   // Weights needed  
		...
		if(Cr2r2<1.e-9) return false;   //Comes in the denumerator; should be positively defined
