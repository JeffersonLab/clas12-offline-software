...
//      allhits = this.readRawHits(event,charge2Energy,timeOffsets,geometry,trkGeo);
    ...
    public ArrayList<FTTRKCluster> findClusters(List<FTTRKHit> hits) {
        ...
        for (FTTRKHit hit : hits) {
            ...
            if (w > 0 && w <= Nstrips) {
                ...
                if (debugMode >= 1) {
                    System.out.println(w + " " + l + " " + HitArray[w - 1][l - 1].printInfo());
                }
            }
        ...
        for (int il = 0; il < Nlayers; il++) {
            ...
            while (is < sequentialStrips) {
                ...
                if (HitArray[ris][il] != null && !checked[ris][il]) {
                    ...
                    ArrayList<FTTRKHit> cloneHitsWNewID = new ArrayList<>();
            ...
            while (is < (FTTRKConstantsLoader.Longstrips + sideShortStrips)) {  //384
                ...
                if (HitArray[lis][il] != null && !checked[lis][il]) {
                    ArrayList<FTTRKHit> clusterHits = new ArrayList<>();
                    checked[lis][il] = true;
                    // if fired this is the first strip of the cluster
                    ...
                    // look for a consecutive strip in the stack and stick it to the cluster
                    int isnext = is + 1;
                    while (isnext < (sequentialStrips + FTTRKConstantsLoader.SideHalfstrips)
                            && // 640 
                ...
                if (clust.get_Layer() == il + 1) {
                    ...
                    for (int i = 0; i < nbhits; i++) {
                        ...
                        if (nstrip == stripDiscontinuity1 || nstrip == stripDiscontinuity2 || nstrip == stripDiscontinuity3 || nstrip == stripDiscontinuity4) {
                            ...
                            if (nstrip == stripDiscontinuity1) {
                                clusterId11 = clust.get_CId();  //cut 127-384
                            }
                            if (nstrip == stripDiscontinuity2) {
                                clusterId12 = clust.get_CId();
                            }
                            if (nstrip == stripDiscontinuity3) {
                                clusterId21 = clust.get_CId();  // cut 640-641
                            }
                            if (nstrip == stripDiscontinuity4) {
            ...
            if (clusterId11 >= 0 && clusterId12 >= 0 && clusterId11 != clusterId12) {
                ArrayList<FTTRKHit> twoClusterHits = new ArrayList<>();
    ...
    public ArrayList<FTTRKCross> findCrosses(List<FTTRKCluster> clusters) {
        ...
        ArrayList<FTTRKCluster> allinnerlayrclus = new ArrayList<>();
        ArrayList<FTTRKCluster> allouterlayrclus = new ArrayList<>();
        ...
        for (FTTRKCluster inlayerclus : allinnerlayrclus) {
            if (inlayerclus.size() < FTConstants.TRK_MIN_CLUS_SIZE) {
                continue;
            }
            for (FTTRKCluster outlayerclus : allouterlayrclus) {
                if (outlayerclus.size() < FTConstants.TRK_MIN_CLUS_SIZE) {
                    continue;
                }
                ...
                if (debugMode >= 1) {
                    System.out.println(inlayerclus.printInfo() + " " + outlayerclus.printInfo());
                }
                if ((inlayerclus.get_MinStrip() + outlayerclus.get_MinStrip() > 0)
                    ...
                    if (debugMode >= 1) {
                        System.out.println("cross radius =============" + radXCenter);
                    }
                    if (radXCenter > FTTRKConstantsLoader.InnerHole && radXCenter < FTTRKConstantsLoader.Rmax) {
                        crosses.add(this_cross);
                    }
                    if (debugMode >= 1) {
                        System.out.println("cross info :" + this_cross.printInfo() + " " + crosses.size());
                    }
...
//////////////////////////////////////////////////////// insert here possible operations selecting strips or groups of strips
        ...
        List<FTTRKHit> hitsOrderedByLay = new ArrayList<>();
        ...
        for (int nlayer = 1; nlayer <= Nlayers; nlayer++) {
            for (FTTRKHit h : hits) {
                if (h.get_Layer() == nlayer) {
                    hitsOrderedByLay.add(h);
                }
            }
        }

        // order by strip
        List<FTTRKHit> hitsOrderedByStrip = new ArrayList<>();
        for (int nlayer = 1; nlayer <= Nlayers; nlayer++) {
            for (int nstrip = 1; nstrip <= Nstrips; nstrip++) {
                for (FTTRKHit h : hitsOrderedByLay) {
                    if (nlayer == h.get_Layer() && nstrip == h.get_Strip()) {
                        hitsOrderedByStrip.add(h);
                    }
                }
            }
        }
...
//                bankHits.setFloat("z",i,(float) (hits.get(i).get_Dz()/10.0));
        ...
        // cluster bank
        if (debugMode >= 1) {
            System.out.println("cluster bank size " + clusters.size());
        }
        if (!clusters.isEmpty()) {
        ...
        // cross bank
        if (debugMode >= 1) {
            System.out.println("crosses bank size " + crosses.size());
        }
        if (!crosses.isEmpty()) {
    ...
    public int reverseStripsInSector(int icomponent) {
        ...
        if (nsector > NStripsSectors) {
            System.out.println("wrong sector number, check code");
        }
    ...
    public int reverseStripInSecondHalf(int icomponent) {
        ...
        int nsector = findSector(icomponent);
        if (nsector > NStripsSectors) {
            System.err.println("wrong FTTRK strip sector number, check code");
        }
        int halfStrip = (sectorLimits[nsector + 1] - sectorLimits[nsector]) / 2 + sectorLimits[nsector];
        if (icomponent > halfStrip) {
            int offset = sectorLimits[nsector + 1] - icomponent;
