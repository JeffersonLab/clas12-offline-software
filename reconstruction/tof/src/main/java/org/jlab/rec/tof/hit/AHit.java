...
public abstract class AHit implements Comparable<AHit> {
	 ...
	 *            the status.
	 * @param v1
	 *            effective velocity (L for FTOF, U for CTOF)
	 * @param v2
	 *            effective velocity (R for FTOF, D for CTOF)
	 * @param v1Unc
	 *            effective velocity uncertainty (L for FTOF, U for CTOF)
	 * @param v2Unc
	 *            effective velocity uncertainty (R for FTOF, D for CTOF)
	 * @param ADC1Err
	 *            ADC (L for FTOF, U for CTOF) Jitter
	 * @param ADC2Err
	 *            ADC (R for FTOF, D for CTOF) Jitter
	 ...
	 * 
	 * @param y
	 *            the hit position in bar coordinate system
	 * @param PED1
	 *            for FTOF L pedestal, for CTOF U pedestal
	 * @param PED2
	 *            for FTOF R pedestal, for CTOF D pedestal
	 */
	 ...
	 * 
	 * @param ADC1Err
	 *            for FTOF L Jitter, for CTOF U Jitter
	 * @param ADC2Err
	 *            for FTOF R Jitter, for CTOF D Jitter
	...
	/**
	 * @param paddle2paddle
	 *            paddle2paddle calibration parameter
	 * @param timeOffset
	 *            timeOffset calibration parameter
	 * @param LSBConv
	 *            LSB conversion factor
	 ...
	 * 
	 * @param TDC1Err
	 *            for FTOF L,U Jitter, for CTOF U Jitter
	 * @param TDC2Err
	 *            for FTOF R,D Jitter, for CTOF D Jitter
	 * @param LSBConv
	 *            LSB conversion factor
	 * @param LSBConvErr
	 *            Uncertainty in LSB conversion factor
	...
	private double[] calc_tErr12(double TDC1Err, double TDC2Err,
		...
		if (Character.toString(this.get_StatusWord().charAt(1)).equals("1")) // TDC1
			...
			tErr1 = Math.sqrt((this.get_TDC1() * LSBConvErr)
					* (this.get_TDC1() * LSBConvErr) + (TDC1Err * LSBConv)
					* (TDC1Err * LSBConv));
		if (Character.toString(this.get_StatusWord().charAt(3)).equals("1")) // TDC2
																				// is
																				// valid
			tErr2 = Math.sqrt((this.get_TDC2() * LSBConvErr)
					* (this.get_TDC2() * LSBConvErr) + (TDC2Err * LSBConv)
					* (TDC2Err * LSBConv));
	 ...
	 *            error in y
	 * @param t1
	 *            for FTOF tL, for CTOF tU
	 * @param t1Unc
	 *            uncertainty in t1 [for FTOF tL, for CTOF tU]
	 * @param v1
	 *            effective velocity [L for FTOF, U for CTOF]
	 * @param v1Unc
	 *            uncertainty in the effective velocity [for FTOF vL, for CTOF
	 *            vU]
	 * @param t2
	 *            for FTOF tR, for CTOF tD
	 * @param t2Unc
	 *            uncertainty in t2 [for FTOF tR, for CTOF tD]
	 * @param v2
	 *            effective velocity [R for FTOF, D for CTOF]
	 * @param v2Unc
	 *            uncertainty in the effective velocity [for FTOF vR, for CTOF
	 *            vD]
	 * @param L
	 *            the paddle length
	 ...
	 *            scintillator bar thickness
	 * @param PEDErr1
	 *            uncertainty in pedestal [for FTOF L, for CTOF U]
	 * @param PEDErr2
	 *            uncertainty in pedestal [for FTOF R, for CTOF D]
	 * @param ADCErr1
	 *            for FTOF L Jitter, for CTOF U Jitter
	 * @param ADCErr2
	 *            for FTOF R Jitter, for CTOF D Jitter
	 ...
	 *            the PMT energy
	 * @param E1Unc
	 *            the uncertainty on the PMT energy
	 * @param E2
	 *            the PMT energy
	 * @param E2Unc
	 *            the uncertainty on the PMT energy
	 * @param lambda1
	 *            lambda (FTOF) L (CTOF) U
	 * @param lambda1Unc
	 *            uncertainty in lambda (FTOF) L (CTOF) U
	 * @param lambda2
	 *            lambda (FTOF) R (CTOF) D
	 * @param lambda2Unc
	 *            uncertainty in lambda (FTOF) R (CTOF) D
