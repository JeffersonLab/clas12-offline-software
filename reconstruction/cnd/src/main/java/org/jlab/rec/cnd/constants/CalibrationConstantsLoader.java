...
public class CalibrationConstantsLoader {
    ...
    private final static String[] cndTables = new String[] { "/calibration/cnd/UturnEloss",
            "/calibration/cnd/UturnTloss", "/calibration/cnd/TimeOffsets_LR", "/calibration/cnd/TDC_conv",
            "/calibration/cnd/TimeOffsets_layer", "/calibration/cnd/EffV", "/calibration/cnd/Attenuation",
            "/calibration/cnd/Status_LR", "/calibration/cnd/Energy", "/calibration/cnd/time_jitter",
            "/geometry/cnd/cndgeom", "/geometry/target", "/calibration/cnd/cluster" };
    ...
    public final void Load(int run, ConstantsManager ccdb) {
        ...
        // load table reads entire table and makes an array of variables for each column
        // in the table.
        ...
        for (int iSec = 1; iSec <= 24; iSec++) {
            for (int iLay = 1; iLay <= 3; iLay++) {
                UTURNELOSS[iSec - 1][iLay - 1] = tabJs.get(0).getDoubleValue("uturn_eloss", iSec, iLay, 1); // component
                                                                                                            // is 1
        ...
        for (int iLay = 1; iLay <= 3; iLay++) {
            ...
            ZOFFSET[iLay - 1] = 10. * tabJs.get(10).getDoubleValue("UpstreamZOffset", 1, iLay, 1);// not right structure
                                                                                                  // for common tools
        }
