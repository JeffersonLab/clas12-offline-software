...
public class CNDClusterFinder {
    ...
    public ArrayList<CNDCluster> findClusters(ArrayList<CndHit> hits) {
        ...
        // loop over hits to group hits in clusters based on the mesure "distance", to
        // be improved
        int j = 0;
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int k = 0; k < j/* clustered_hits.size() */; k++) {
                ...
                for (int l = 0; l < 1/* clustered_hits.getItem(k).size() */; l++) {
                    ...
                    if (deltaX/10.0 < 20 && deltaY/10.0 < 20 && deltaZ/10.0 < 20 && deltaT < 10) {
        ...
        /*
         * /// calculate the Edep-weighted average for(int i = 0; i < clusters_x.size();
         * i++){ clusters_x.set(i,
         * clusters_xTimesEdep.get(i)/clusters_energysum.get(i)); clusters_y.set(i,
         * clusters_yTimesEdep.get(i)/clusters_energysum.get(i)); clusters_z.set(i,
         * clusters_zTimesEdep.get(i)/clusters_energysum.get(i)); clusters_time.set(i,
         * clusters_timeTimesEdep.get(i)/clusters_energysum.get(i)); }
         */
    ...
    /// unit : cm
    private double sigmaX(double x) {
        return 1.6;
    }

    /// unit : cm
    private double sigmaY(double y) {
        return 1.6;
    }
    ...
    /// unit : ns
    private double sigmaTime(double t) {
        return 0.14;
    }

    /// unit : deg.
    private double sigmaTheta(double theta) {
        return 2.5;
    }

    /// unit : deg.
    private double sigmaPhi(double phi) {
        return 2.8;
    }

    /// beta = v/c.
    private double sigmaBeta(double beta) {
        return 0.065 * beta;
    }

    /// find the two closest hits among all the hits
    /*
     * private void find_closest(int begin, ArrayList<Double> x, ArrayList<Double>
     * y, ArrayList<Double> z, ArrayList<Double> time, int[] subA, int[] subB,
     * double[] closest_distance){ if((begin+1)>=x.size())return;
     * 
     * 
     * 
     * for(int i=begin+1;i<x.size();i++){ double distance = sqrt(
     * (x.get(begin)-x.get(i))*(x.get(begin)-x.get(i))/sigmaX(x.get(begin))/sigmaX(x
     * .get(i))
     * +(y.get(begin)-y.get(i))*(y.get(begin)-y.get(i))/sigmaY(y.get(begin))/sigmaY(
     * y.get(i))
     * +(z.get(begin)-z.get(i))*(z.get(begin)-z.get(i))/sigmaZ(z.get(begin))/sigmaZ(
     * z.get(i))
     * +(time.get(begin)-time.get(i))*(time.get(begin)-time.get(i))/sigmaTime(time.
     * get(begin))/sigmaTime(time.get(i)) );
     * 
     * 
     * if(distance > cluster_size_){ // continue; } else{
     * if(distance<closest_distance[0]){ subA[0] = begin ; subB[0] = i ;
     * closest_distance[0] = distance; } } } find_closest(begin+1, x, y, z, time,
     * subA, subB, closest_distance);//recursive. Hirarchical clustering }
     */
