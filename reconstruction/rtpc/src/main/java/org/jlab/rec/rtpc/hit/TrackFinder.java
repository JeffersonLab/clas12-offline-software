...
public class TrackFinder {
  ...
  public TrackFinder(HitParameters params, boolean cosmic) {
    ...
    for (int time = 0;
        time < TrigWindSize;
        time += StepSize) { // Steps of 120 up to TrigWindSize = 10000
      ...
      for (int padindex = 0; padindex < padloopsize; padindex++) {
        ...
        if (adc > adcthresh) { // pad adc threshold check
          ...
          for (int tid : TIDList) {
            ...
            for (int timecheck = time;
                timecheck > 0 && timecheck >= time - timeadjlimit * StepSize;
                timecheck -= StepSize) {
              ...
              if (!padlist.contains(pad)) { // Ensures pad isn't already assigned here
                ...
                for (int checkpad : padlist) {
                  ...
                  if (tutil.comparePads(
                      ...
                      zthreshgap,
                      phithresh,
                      phithreshgap)) { // compares the position of two pads
              ...
              } else { // pad is somehow in this time slice already so lets go ahead and add it to
    ...
    for (int tid :
        TIDMap
            .getAllTrackIDs()) { // We need to remove tracks with not enough pads to save time later
        ...
        for (int tx : times) {
          ...
          if (pads.size() > 1) {
            ...
            for (int index = 1; index < pads.size(); index++) {
              ...
              if (!tutil.comparePads(
                  pparent, p, method, cosmic, zthresh, zthreshgap, phithresh, phithreshgap)) {
                t.flagTrack();
                break TRACKTIMELOOP;
              }
            }
