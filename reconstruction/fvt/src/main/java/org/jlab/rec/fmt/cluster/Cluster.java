...
/**
 * A cluster in the fmt consists of an array of hits that are grouped together according to the
 * algorithm of the ClusterFinder class
 * @author ziegler
 */
public class Cluster extends ArrayList<FittedHit> implements Comparable<Cluster>{
	...
	public String printInfo() {
		return "fmt cluster: ID " + this.get_Id() + " Seed " + this.get_SeedStrip()
                + " Layer " + this.get_Layer() + " Size " + this.size();
	}
	...
	/**
	 * Sets energy-weighted parameters; these are the strip centroid (energy-weighted) value, the
     * energy-weighted phi for Z detectors and the energy-weighted z for C detectors
	 */
