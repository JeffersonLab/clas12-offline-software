...
public class NoiseReductionParameters {
    ...
    private static boolean _lookForTracks = true;
     ...
     * 
     * @param allowedMissingLayers
     *            the number of missing layers permitted.
     * @param leftLayerShifts
     *            the shifts for left leaning tracks. Length should equal
     *            numLayers.
     * @param rightLayerShifts
     *            the shifts for right leaning tracks. Length should equal
     *            numLayers.
     */
     ...
     * 
     * @param packedData
     *            the packedData to set. This should be new raw data.
     */
     ...
     *            benders
     * @param compositeData
     *            this will be a "layer" from the composite detector after it
     *            (the composite detector) has gone through its own 1-pass noise
     *            removal. What happens is those segments from the real chamber
     *            that correspond to noise in the composite chamber are removed
     *            and the real data is refiltered (cleaned) from the reduced
     *            segments
     */
	...
	if (opt == LEFT_LEAN) {
	    ...
	    ExtendedWord
		    .bitwiseAnd(rightSegments, compositeData, rightSegments);
	}
	cleanFromSegments();
    }
	...
	if (direction == LEFT_LEAN) {
		...
		ExtendedWord
			.copy(parameters._packedData[i], parameters.copy[i]);
     ...
     * 
     * @param lookForTracks
     *            flag determining if a second pass to look for tracks is used.
     */
